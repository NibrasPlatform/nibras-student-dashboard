window.NibrasReact.run(() => {
    ...
    const initializeGoogleAuth = async () => {
        ...
        try {
            ...
            console.log('Google gsi loaded, client ID:', googleClientId);
            
            if (!window.google?.accounts?.oauth2) {
                throw new Error('Google OAuth2 API not available');
            }
            ...
            googleSignInContainer.hidden = true;
            const errorMsg = error?.message || error?.error_details || JSON.stringify(error);
            setGoogleStatus(errorMsg, 'error');
        }
    };

...
function updateSidebarUser() {
    try {
        ...
        if (bioEl) bioEl.textContent = role ? role + ' — Completed projects and contributions' : 'Completed projects and contributions';
    } catch (_) {}
}
...
function loadPortfolio() {
    ...
    grid.innerHTML = '<div class="portfolio-loading"><i class="fa-solid fa-circle-notch fa-spin" style="font-size:1.5rem;display:block;margin-bottom:12px;"></i>Loading portfolio...</div>';
    ...
    if (svc && svc.projectService && userId) {
        fetchPromise = svc.projectService.listByInstructor({ userId: userId, limit: 100 }).catch(function () {
    ...
    fetchPromise.then(function (projects) {
    ...
    return [
        {
            ...
            description: 'Built a full-stack e-commerce platform with React, Node.js, and PostgreSQL. Features include user authentication, product catalog, shopping cart, and payment integration with Stripe.',
            status: 'completed',
            grade: '92',
        ...
        },
        {
            ...
            description: 'Created an interactive data visualization dashboard using Python, Pandas, and D3.js. Analyzed 100K+ records to uncover trends in educational outcomes.',
            status: 'completed',
            grade: '88',
        ...
        },
        {
            ...
            description: 'Developed a cross-platform mobile task management application with real-time sync, push notifications, and team collaboration features.',
            status: 'completed',
            grade: '95',
        ...
        },
        {
            ...
            description: 'Integrated GPT-based chatbot into an existing web application with context-aware responses, sentiment analysis, and learning analytics tracking.',
            status: 'in_progress',
...
function applyFilters() {
    var filtered = portfolioState.projects.filter(function (p) {
        var matchesSearch = !portfolioState.searchTerm
            || (p.title && p.title.toLowerCase().includes(portfolioState.searchTerm))
            || (p.description && p.description.toLowerCase().includes(portfolioState.searchTerm))
            || (p.courseName && p.courseName.toLowerCase().includes(portfolioState.searchTerm))
            || (p.techStack && p.techStack.some(function (t) { return t.toLowerCase().includes(portfolioState.searchTerm); }));
        var matchesStatus = portfolioState.statusFilter === 'all' || p.status === portfolioState.statusFilter;
...
function renderPortfolioGrid(projects) {
    ...
    var completed = portfolioState.projects.filter(function (p) { return p.status === 'completed'; }).length;
    var grades = portfolioState.projects.filter(function (p) { return p.grade; }).map(function (p) { return parseFloat(p.grade); });
    var avgGrade = grades.length ? Math.round(grades.reduce(function (a, b) { return a + b; }, 0) / grades.length) : '--';

    document.getElementById('stat-projects').textContent = total;
    document.getElementById('stat-completed').textContent = completed;
    document.getElementById('stat-grades').textContent = avgGrade + (avgGrade !== '--' ? '%' : '');

    grid.innerHTML = projects.map(function (p) {
        var techTags = (p.techStack || []).map(function (t) {
            return '<span class="tech-tag">' + escapeHtml(t) + '</span>';
        }).join('');
        var statusClass = p.status === 'completed' ? 'status-completed' : 'status-in_progress';
        var statusLabel = p.status === 'completed' ? 'Completed' : 'In Progress';
        var gradeDisplay = p.grade ? '<span class="portfolio-card-grade">' + escapeHtml(p.grade) + '%</span>' : '';
        var dateDisplay = p.completedDate
            ? formatDate(p.completedDate)
            : (p.startDate ? 'Started ' + formatDate(p.startDate) : '');

        return '<div class="portfolio-card" data-id="' + escapeHtml(p.id) + '" onclick="openPortfolioDetail(\'' + escapeHtml(p.id) + '\')">'
            + '<div class="portfolio-card-header">'
            + '<span class="portfolio-card-title">' + escapeHtml(p.title) + '</span>'
            + '<span class="portfolio-card-status ' + statusClass + '">' + statusLabel + '</span>'
            + '</div>'
            + '<div class="portfolio-card-course">' + escapeHtml(p.courseName || '') + '</div>'
            + '<div class="portfolio-card-desc">' + escapeHtml(p.description || '') + '</div>'
            + (techTags ? '<div class="portfolio-card-tech">' + techTags + '</div>' : '')
            + '<div class="portfolio-card-footer">'
            + '<span>' + gradeDisplay + '</span>'
            + '<span class="portfolio-card-date">' + dateDisplay + '</span>'
            + '</div></div>';
    }).join('');
}

function openPortfolioDetail(projectId) {
    var project = portfolioState.filteredProjects.find(function (p) { return p.id === projectId; });
    if (!project) return;
    ...
    document.getElementById('portfolio-detail-title').textContent = project.title || 'Project Details';

    // Description
    document.getElementById('portfolio-detail-description').innerHTML = '<h4>Description</h4><p>' + escapeHtml(project.description || 'No description available.') + '</p>';

    // Meta
    var metaHtml = '<h4>Details</h4>';
    if (project.courseName) metaHtml += '<div class="detail-value"><i class="fa-solid fa-book-open"></i> ' + escapeHtml(project.courseName) + '</div>';
    if (project.grade) metaHtml += '<div class="detail-value"><i class="fa-solid fa-star"></i> Grade: ' + escapeHtml(project.grade) + '%</div>';
    if (project.teamSize) metaHtml += '<div class="detail-value"><i class="fa-solid fa-users"></i> Team size: ' + project.teamSize + '</div>';
    if (project.completedDate) metaHtml += '<div class="detail-value"><i class="fa-regular fa-calendar-check"></i> Completed: ' + formatDate(project.completedDate) + '</div>';
    else if (project.startDate) metaHtml += '<div class="detail-value"><i class="fa-regular fa-calendar"></i> Started: ' + formatDate(project.startDate) + '</div>';
    ...
    document.getElementById('portfolio-detail-techstack').innerHTML = techs.length
        ? '<h4>Tech Stack</h4><div class="detail-tags">' + techs.map(function (t) { return '<span class="detail-tag">' + escapeHtml(t) + '</span>'; }).join('') + '</div>'
        : '';

    // Team
    var members = project.teamMembers || [];
    document.getElementById('portfolio-detail-team').innerHTML = members.length
        ? '<h4>Team</h4>' + members.map(function (m) { return '<div class="detail-team-member"><i class="fa-solid fa-user"></i> ' + escapeHtml(m) + '</div>'; }).join('')
        : '<h4>Team</h4><p class="detail-value">Individual project</p>';

    // Links
    var linksHtml = '';
    if (project.githubUrl) linksHtml += '<a href="' + escapeHtml(project.githubUrl) + '" target="_blank" class="detail-link"><i class="fa-brands fa-github"></i> ' + escapeHtml(project.githubUrl) + '</a>';
    if (project.liveUrl) linksHtml += '<a href="' + escapeHtml(project.liveUrl) + '" target="_blank" class="detail-link"><i class="fa-solid fa-globe"></i> ' + escapeHtml(project.liveUrl) + '</a>';
    document.getElementById('portfolio-detail-links').innerHTML = linksHtml ? '<h4>Links</h4>' + linksHtml : '';

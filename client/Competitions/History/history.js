(window.NibrasReact && typeof window.NibrasReact.run === 'function'
    ...
    const renderDelta = (delta) => {
        ...
        if (num > 0) return '<span class="delta-positive"><i class="fa-solid fa-caret-up"></i> +' + num + '</span>';
        if (num < 0) return '<span class="delta-negative"><i class="fa-solid fa-caret-down"></i> ' + num + '</span>';
    ...
    const renderStats = () => {
        ...
        const bestRank = state.items.reduce((best, item) => {
            ...
            return !Number.isNaN(rank) && rank > 0 && (best === null || rank < best) ? rank : best;
        }, null);
        const stats = [
            { label: 'Total Participations', value: String(total), icon: 'fa-solid fa-users', color: 'blue' },
            { label: 'Codeforces', value: String(codeforcesCount), icon: 'fa-solid fa-code', color: 'green' },
            { label: 'LeetCode', value: String(leetcodeCount), icon: 'fa-solid fa-laptop-code', color: 'yellow' },
            { label: 'Best Rank', value: bestRank ? '#' + bestRank : '—', icon: 'fa-solid fa-trophy', color: 'purple' },
        ];
    ...
    const renderHistory = () => {
        ...
        state.items.forEach((item) => {
            ...
            const solved = !Number.isNaN(Number(item.solved)) ? Number(item.solved) : (!Number.isNaN(Number(item.problemsSolved)) ? Number(item.problemsSolved) : null);
            const totalProblems = !Number.isNaN(Number(item.totalProblems)) ? Number(item.totalProblems) : null;
            const rating = !Number.isNaN(Number(item.rating)) ? Number(item.rating) : null;
            const delta = item.delta !== null && item.delta !== undefined && !Number.isNaN(Number(item.delta)) ? Number(item.delta) : null;
    ...
    const renderPagination = () => {
        ...
        paginationEl.querySelectorAll('[data-page]').forEach((btn) => {
            btn.addEventListener('click', () => {
                ...
                else if (page === 'next' && currentPage < totalPages) goToPage(currentPage + 1);
    ...
    const renderPageNumbers = (current, total) => {
        ...
        if (start > 1) {
            pages.push('<button class="pagination-btn pagination-page" data-page="1">1</button>');
            if (start > 2) pages.push('<span class="pagination-ellipsis">...</span>');
        }

        for (let i = start; i <= end; i++) {
            pages.push('<button class="pagination-btn pagination-page' + (i === current ? ' active' : '') + '" data-page="' + i + '">' + i + '</button>');
        }

        if (end < total) {
            if (end < total - 1) pages.push('<span class="pagination-ellipsis">...</span>');
            pages.push('<button class="pagination-btn pagination-page" data-page="' + total + '">' + total + '</button>');
        }
    ...
    const loadHistory = async () => {
        ...
        try {
            const response = await competitionsService.listHistory({ page: state.currentPage, limit: state.limit });

window.NibrasReact.run(function () {
    ...
    function renderHeader() {
        ...
        els.title.textContent = contestData.name || contestData.title || 'Untitled Contest';
    ...
    function renderProblemsList() {
        if (!els.problemsList || problems.length === 0) {
            els.problemsList.innerHTML = '<div class="loading-state">No problems available.</div>';
            return;
        }
        ...
        problems.forEach(function (p, i) {
            ...
            if (status === 'solved' || status === 'accepted') {
                statusIcon = '<span class="prob-status-icon solved"><i class="fa-regular fa-circle-check"></i></span>';
                solvedCount++;
            } else if (status === 'attempted') {
                statusIcon = '<span class="prob-status-icon attempted"><i class="fa-regular fa-circle-xmark"></i></span>';
            } else {
                statusIcon = '<span class="prob-status-icon"><i class="fa-regular fa-circle"></i></span>';
            }

            html += '<div class="problem-list-item' + (i === selectedProblemIndex ? ' active' : '') + '" data-index="' + i + '">';
            html += '<span class="prob-letter">' + letter + '</span>';
            html += statusIcon;
            html += '<span class="prob-name">' + esc(title) + '</span>';
            html += '<span class="prob-difficulty ' + esc(difficulty) + '">' + esc(difficulty) + '</span>';
            html += '</div>';
        });
    ...
    function selectProblem(index) {
        ...
        els.problemTitle.textContent = getProblemLetter(index) + '. ' + (problem.title || problem.name || 'Problem');
        els.problemMeta.innerHTML = '';
        var difficulty = problem.difficulty || 'medium';
        els.problemMeta.innerHTML += '<span><i class="fa-solid fa-signal"></i> ' + esc(difficulty) + '</span>';
        if (problem.timeLimit) els.problemMeta.innerHTML += '<span><i class="fa-regular fa-clock"></i> ' + esc(problem.timeLimit) + 'ms</span>';
        if (problem.memoryLimit) els.problemMeta.innerHTML += '<span><i class="fa-solid fa-memory"></i> ' + esc(problem.memoryLimit) + 'MB</span>';
        if (problem.points) els.problemMeta.innerHTML += '<span><i class="fa-solid fa-star"></i> ' + esc(problem.points) + ' pts</span>';
        if (Array.isArray(problem.tags) && problem.tags.length > 0) {
            problem.tags.forEach(function (t) {
                els.problemMeta.innerHTML += '<span style="padding:2px 8px;border-radius:4px;background:var(--tag-bg);font-size:0.78rem;">' + esc(typeof t === 'string' ? t : (t.name || t)) + '</span>';
            });
        }
        ...
        if (Array.isArray(samples) && samples.length > 0) {
            samples.forEach(function (s, i) {
                ...
                els.problemSample.innerHTML += '<div class="sample-label">Sample ' + (i + 1) + ':</div>';
                if (input) els.problemSample.innerHTML += '<div class="sample-box"><strong>Input:</strong>\n' + esc(input) + '</div>';
                if (output) els.problemSample.innerHTML += '<div class="sample-box"><strong>Output:</strong>\n' + esc(output) + '</div>';
            });
        }
    ...
    function loadSubmissions() {
        ...
        if (currentUserIdVal) query += '&userId=' + encodeURIComponent(currentUserIdVal);

        competitionsService.listContests({}).then(function () {
    ...
    function fetchSubmissionsFallback(query) {
        ...
        fetch(url, {
            ...
            var list = data?.data || data?.submissions || (Array.isArray(data) ? data : []);
    ...
    function renderSubmissions() {
        ...
        if (submissions.length === 0) {
            els.submissionsBody.innerHTML = '<tr><td colspan="6" class="table-empty">No submissions yet.</td></tr>';
            return;
        }
        ...
        submissions.forEach(function (s) {
            ...
            var probName = s.problemName || s.problemTitle || (probIndex >= 0 ? problems[probIndex].title : 'Problem ' + letter);
            var lang = s.language || 'N/A';
            var status = s.status || 'pending';
            var statusClass = 'status-pending';
            if (status === 'accepted' || status === 'AC' || status === 'correct') { statusClass = 'status-accepted'; status = 'Accepted'; }
            else if (status === 'wrong' || status === 'WA') { statusClass = 'status-rejected'; status = 'Wrong Answer'; }
            else if (status === 'tle' || status === 'TLE') { statusClass = 'status-rejected'; status = 'Time Limit Exceeded'; }
            else if (status === 'mle' || status === 'MLE') { statusClass = 'status-rejected'; status = 'Memory Limit Exceeded'; }
            else if (status === 're' || status === 'RE' || status === 'error') { statusClass = 'status-rejected'; status = 'Runtime Error'; }
            else if (status === 'pending' || status === 'PENDING' || status === 'queue') { statusClass = 'status-pending'; status = 'Pending...'; }
    ...
    function submitSolution() {
        ...
        fetch(url, {
            ...
            var submissionId = data._id || data.id || data.submission?._id || data.submission?.id;
            if (submissionId) {
    ...
    function pollSubmissionStatus(submissionId) {
        ...
        function poll() {
            attempt++;
            var url = apiUrl.replace(/\/+$/, '') + '/submissions/' + encodeURIComponent(submissionId);
            fetch(url, {
                ...
                if (attempt < maxAttempts) {
                    ...
                    els.submissionStatus.textContent = 'Result pending... check submissions tab.';
                    els.submissionStatus.style.color = 'var(--text-secondary)';
                }
            }).catch(function () {
    ...
    function getApiBaseUrl() {
        ...
        return window.NIBRAS_API_URL || window.NIBRAS_BACKEND_URL || 'https://nibras-backend.up.railway.app/api';
    }
    ...
    function loadLeaderboard() {
        ...
        if (!contestData) {
            els.leaderboardBody.innerHTML = '<tr><td colspan="5" class="table-empty">Contest data not loaded.</td></tr>';
            return;
        }
        ...
        if (standingsData.length === 0) {
            els.leaderboardBody.innerHTML = '<tr><td colspan="5" class="table-empty">No participants yet.</td></tr>';
            return;
        }
    ...
    function renderLeaderboard(entries) {
        ...
        entries.forEach(function (entry, i) {
            ...
            var userName = entry.userName || entry.name || entry.user?.name || 'User ' + rank;
            var userId = entry.userId || entry.user?._id || entry.user?.id || '';
            var isCurrentUser = userId && currentUserId && (userId === currentUserId || userId.toString() === currentUserId.toString());
            var solved = entry.solved || entry.problemsSolved || entry.score || 0;
            var penalty = entry.penalty || entry.penaltyTime || (entry.totalTime != null ? entry.totalTime : '—');
            ...
            html += '<td class="col-user"><span class="user-avatar-small">' + userInitials + '</span><span class="user-name-display">' + esc(userName) + '</span></td>';
            html += '<td>' + solved + '</td>';
            html += '<td>' + (penalty !== '—' && penalty != null ? penalty : '—') + '</td>';
    ...
    function initSocket() {
        ...
        try {
            var backendUrl = getApiBaseUrl().replace(/\/api\/?$/, '').replace(/\/+$/, '');
            ...
            socket.on('submission:status', function (data) {
                if (data && (data.contestId === contestId || !data.contestId)) {
                    ...
                    if (probIdx >= 0 && data.status) {
                        ...
                        } else if (['wrong', 'wa', 'tle', 'mle', 're'].indexOf(statusLower) !== -1) {
    ...
    try {
        var user = JSON.parse(localStorage.getItem('user') || '{}');
        var initials = user.name ? getInitials(user.name) : (user.email ? user.email[0].toUpperCase() : '?');
        document.querySelectorAll('.avatar-circle, .profile-circle-small').forEach(function (el) {
    ...
    if (!contestId) {
        ...
        els.problemsList.innerHTML = '<div class="loading-state">Add ?id=CONTEST_ID to the URL to view a contest.</div>';
        return;
    }

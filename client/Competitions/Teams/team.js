window.NibrasReact.run(function () {
    ...
    function showFeedback(message, tone) {
        var notice = document.getElementById('feedback-notice') || (function () {
            var n = document.createElement('div');
            n.id = 'feedback-notice';
            n.style.cssText = 'margin-bottom:14px;padding:10px 14px;border-radius:6px;display:none;font-size:0.9rem;';
    ...
    function renderTeamCard(team, showActions) {
        var memberCount = Array.isArray(team.members) ? team.members.length : (team.memberCount || 0);
        var maxMembers = team.maxMembers || 4;
        var isOpen = team.openJoin !== false;
        var contestName = team.contestName || (team.contestId && typeof team.contestId === 'object' ? team.contestId.title : null) || '';

        return '<div class="team-card" data-id="' + esc(team._id || team.id) + '">' +
            '<div class="team-card-header">' +
                '<div class="team-card-avatar">' + getInitials(team.name) + '</div>' +
                '<div class="team-card-title">' +
                    '<h4>' + esc(team.name || 'Unnamed Team') + '</h4>' +
                    '<span class="team-card-meta">' +
                        '<i class="fa-solid fa-user"></i> ' + memberCount + '/' + maxMembers +
                        (contestName ? ' &middot; <i class="fa-solid fa-trophy"></i> ' + esc(contestName) : '') +
                    '</span>' +
                '</div>' +
            '</div>' +
            (team.description ? '<p class="team-card-desc">' + esc(team.description) + '</p>' : '') +
            '<div class="team-card-footer">' +
                '<span class="team-status ' + (isOpen ? 'team-status-open' : 'team-status-closed') + '">' +
                    (isOpen ? '<i class="fa-solid fa-unlock"></i> Open' : '<i class="fa-solid fa-lock"></i> Approval') +
                '</span>' +
    ...
    function renderTeamActions(team) {
        ...
        if (isOwner) {
            buttons += '<button class="team-action-btn team-action-manage" data-action="manage" data-id="' + esc(teamId) + '"><i class="fa-solid fa-gear"></i> Manage</button>';
        } else if (isMember) {
            buttons += '<button class="team-action-btn team-action-leave" data-action="leave" data-id="' + esc(teamId) + '"><i class="fa-solid fa-sign-out-alt"></i> Leave</button>';
        } else {
            if (team.openJoin !== false) {
                buttons += '<button class="team-action-btn team-action-join" data-action="join" data-id="' + esc(teamId) + '"><i class="fa-solid fa-user-plus"></i> Join</button>';
            } else {
                buttons += '<button class="team-action-btn team-action-request" data-action="request" data-id="' + esc(teamId) + '"><i class="fa-solid fa-paper-plane"></i> Request</button>';
            }
        }
        buttons += '<button class="team-action-btn team-action-view" data-action="view" data-id="' + esc(teamId) + '"><i class="fa-solid fa-eye"></i> View</button>';
        return buttons;
    }

    function renderInvitationCard(invitation) {
        var team = invitation.team || invitation.teamId || {};
        var teamId = team._id || team.id || invitation.teamId?._id || invitation.teamId?.id || invitation._id;
        var teamName = team.name || invitation.teamName || 'Unknown Team';
        var fromUser = invitation.fromUser || invitation.invitedBy || {};
        var fromName = fromUser.name || fromUser.username || fromUser.email || 'Someone';
        var status = invitation.status || 'pending';

        return '<div class="team-card invitation-card" data-id="' + esc(teamId) + '" data-invite-id="' + esc(invitation._id || invitation.id) + '">' +
            '<div class="team-card-header">' +
                '<div class="team-card-avatar invite-avatar">' + getInitials(teamName) + '</div>' +
                '<div class="team-card-title">' +
                    '<h4>' + esc(teamName) + '</h4>' +
                    '<span class="team-card-meta"><i class="fa-solid fa-user"></i> Invited by ' + esc(fromName) + '</span>' +
                '</div>' +
            '</div>' +
            '<div class="invitation-actions">' +
                '<button class="invite-btn invite-accept" data-action="accept-invite" data-team-id="' + esc(teamId) + '" data-invite-id="' + esc(invitation._id || invitation.id) + '"><i class="fa-solid fa-check"></i> Accept</button>' +
                '<button class="invite-btn invite-decline" data-action="decline-invite" data-team-id="' + esc(teamId) + '" data-invite-id="' + esc(invitation._id || invitation.id) + '"><i class="fa-solid fa-times"></i> Decline</button>' +
            '</div>' +
        '</div>';
    }
    ...
    function renderMyTeams() {
        ...
        els.myTeamsGrid.innerHTML = myTeams.map(function (t) { return renderTeamCard(t, true); }).join('');
    }
    ...
    function renderDiscover() {
        ...
        els.discoverGrid.innerHTML = discoveredTeams.map(function (t) { return renderTeamCard(t, true); }).join('');
    }
    ...
    function renderInvitations() {
        ...
        els.invitationsGrid.innerHTML = pendingInvitations.map(renderInvitationCard).join('');
    }

    function refreshStats() {
        var myTeamVal = myTeams.length;
        var openTeamVal = discoveredTeams.filter(function (t) { return t.openJoin !== false; }).length;
        ...
        if (els.pendingInviteCount) els.pendingInviteCount.textContent = String(inviteVal);
    ...
    function loadMyTeams() {
        ...
        teamService.listMyTeams({ limit: 50 }).then(function (result) {
    ...
    function loadDiscover() {
        if (!teamService) return;
        teamService.listTeams({ limit: 50 }).then(function (result) {
    ...
    function loadInvitations() {
        ...
        teamService.listMyInvitations({ limit: 50 }).then(function (result) {
            pendingInvitations = Array.isArray(result.invitations) ? result.invitations : [];
            refreshAll();
        }).catch(function () {
            pendingInvitations = [];
            refreshAll();
    ...
    function loadContestsForSelect() {
        ...
        competitionsService.listContests({ limit: 100 }).then(function (result) {
            var contests = Array.isArray(result.contests) ? result.contests : [];
            contests.forEach(function (c) {
                var opt = document.createElement('option');
                opt.value = c._id || c.id || '';
                opt.textContent = (c.title || 'Untitled Contest') + ' (' + (c.platform || 'N/A') + ')';
    ...
    function handleCreateTeamSubmit(e) {
        ...
        teamService.createTeam(data).then(function () {
    ...
    function openTeamDetail(teamId) {
        ...
        els.teamDetailMembersList.innerHTML = '<div class="loading-state"><i class="fa-solid fa-spinner fa-spin"></i> Loading members...</div>';
        ...
        teamService.getTeamById(teamId).then(function (team) {
            ...
            els.teamDetailDescription.textContent = team.description || 'No description.';
            els.teamDetailDescription.style.display = team.description ? 'block' : 'none';

            var memberList = Array.isArray(team.members) ? team.members : [];
            var memberCount = memberList.length;
            var maxMembers = team.maxMembers || 4;
            els.teamDetailMembers.innerHTML = '<i class="fa-solid fa-user"></i> Members: <strong>' + memberCount + '/' + maxMembers + '</strong>';
            els.teamDetailContest.innerHTML = '<i class="fa-solid fa-trophy"></i> Contest: <strong>' + esc(team.contestName || (team.contestId && typeof team.contestId === 'object' ? team.contestId.title : null) || 'None') + '</strong>';
            els.teamDetailStatus.innerHTML = '<i class="fa-solid fa-toggle-on"></i> Status: <strong>' + (team.openJoin !== false ? 'Open' : 'Approval Required') + '</strong>';

            els.teamDetailMembersList.innerHTML = memberList.map(function (m) {
                var user = m.userId || m;
                var userName = user.name || user.username || user.email || 'Unknown';
                var role = m.role || (team.createdBy && (team.createdBy._id === (user._id || user.id) || team.createdBy === (user._id || user.id)) ? 'Owner' : 'Member');
                return '<div class="team-member-row">' +
                    '<div class="member-avatar">' + getInitials(userName) + '</div>' +
                    '<div class="member-info">' +
                        '<span class="member-name">' + esc(userName) + '</span>' +
            ...
            var isMember = (team.members || []).some(function (m) {
                var uid = (m.userId && (m.userId._id || m.userId.id)) || m._id || m.id;
                return uid === currentUserId;
            });
            var isOwner = team.createdBy && ((team.createdBy._id || team.createdBy.id || team.createdBy) === currentUserId);

            actionsHtml += '<button class="btn-primary team-detail-btn" data-action="close-detail" style="margin-right:8px;"><i class="fa-solid fa-chevron-left"></i> Back</button>';

            if (isOwner) {
                actionsHtml += '<button class="btn-danger team-detail-btn" data-action="delete" data-id="' + esc(teamIdVal) + '"><i class="fa-solid fa-trash"></i> Disband Team</button>';
            } else if (isMember) {
                actionsHtml += '<button class="btn-secondary team-detail-btn" data-action="leave" data-id="' + esc(teamIdVal) + '"><i class="fa-solid fa-sign-out-alt"></i> Leave Team</button>';
            } else if (team.openJoin !== false) {
                actionsHtml += '<button class="btn-primary team-detail-btn" data-action="join" data-id="' + esc(teamIdVal) + '"><i class="fa-solid fa-user-plus"></i> Join Team</button>';
            } else {
                actionsHtml += '<button class="btn-primary team-detail-btn" data-action="request" data-id="' + esc(teamIdVal) + '" disabled>Join requested</button>';
            }
    ...
    function handleTeamAction(action, teamId) {
        ...
        if (action === 'join') {
            teamService.joinTeam(teamId).then(function () {
            ...
            teamService.leaveTeam(teamId).then(function () {
            ...
            teamService.deleteTeam(teamId).then(function () {
    ...
    function handleInviteAction(action, teamId, inviteId) {
        ...
        teamService.respondToInvite(teamId, accept).then(function () {
            showFeedback(accept ? 'Invitation accepted!' : 'Invitation declined.', 'success');
            loadInvitations();
            loadMyTeams();
        }).catch(function (err) {
            showFeedback(err?.message || 'Failed to respond to invitation.', 'error');
        });
    }
    ...
    function getCurrentUserInfo() {
        ...
        var stored;
        try { stored = JSON.parse(localStorage.getItem('user') || 'null'); } catch (_) { stored = null; }
        if (stored) {
    ...
    function handleSearch() {
        var query = (els.teamSearchInput ? els.teamSearchInput.value.trim().toLowerCase() : '');
        if (!query) {
            renderDiscover();
            return;
        }
        ...
        if (!filtered.length) {
            ...
            if (els.discoverEmpty) {
                els.discoverEmpty.style.display = 'block';
                els.discoverEmpty.innerHTML = '<i class="fa-solid fa-compass"></i><p>No teams match "' + esc(query) + '".</p>';
            }
            return;
        }
        els.discoverEmpty.style.display = 'none';
        els.discoverGrid.style.display = 'grid';
        els.discoverGrid.innerHTML = filtered.map(function (t) { return renderTeamCard(t, true); }).join('');
    }

window.NibrasReact.run(() => {
    ...
    const backendCourseId = data?.backendCourseId || selectedCourse?.adminCourseId || selectedCourse?.backendCourseId;
    ...
    function updateThemeBtn(theme) {
        ...
        if (theme === 'dark') { themeIcon.className = 'fa-solid fa-sun'; themeText.textContent = 'Light Mode'; }
    ...
    if (crumbLink) crumbLink.setAttribute("href", window.NibrasCourses.withCourseId("../Assignments.html", courseId));
    ...
    if (fileInput) {
        fileInput.addEventListener('change', () => {
            ...
            if (files.length === 0) { fileListNames.textContent = ''; return; }
    ...
    if (btnSubmitGithub) {
        btnSubmitGithub.addEventListener('click', async () => {
            ...
            if (!repoUrl) { alert('Please enter a valid GitHub repository URL.'); return; }
            try {
    ...
    function startPolling(sid) {
        ...
        pollingInterval = setInterval(async () => {
            ...
            try {
                ...
                const evalStatus = String(results?.evaluationStatus || results?.data?.evaluationStatus || status);

                updatePulseUI(evalStatus);

                if (evalStatus === 'completed' || evalStatus === 'passed' || evalStatus === 'approved' || evalStatus === 'needs_review' || evalStatus === 'failed' || evalStatus === 'error') {
                    clearInterval(pollingInterval);
                    if (evalStatus === 'completed' || evalStatus === 'passed' || evalStatus === 'approved') {
    ...
    function updatePulseUI(status) {
        ...
        Object.values(steps).forEach(el => { if (el) el.classList.remove('step-active', 'step-done', 'step-failed'); });
        ...
        } else if (s === 'completed' || s === 'passed' || s === 'approved' || s === 'needs_review') {
            Object.values(steps).forEach(el => { if (el) el.classList.add('step-done'); });
        } else if (s === 'failed' || s === 'error') {
            const current = document.querySelector('.pulse-step.step-active') || steps.init;
            if (current) { current.classList.remove('step-active'); current.classList.add('step-failed'); }
        }
    }
    ...
    async function renderTestResults(sid) {
        ...
        try {
            ...
            if (ev && ev.getResults) {
                ...
                testCases = res?.data?.testCases || res?.testCases || res?.data?.results || [];
            }
            ...
            testCases.forEach((tc, idx) => {
                const isPass = tc.status === 'passed' || tc.passed === true || tc.status === 'pass';
                ...
                container.innerHTML += [
                    ...
                    '<div class="tc-header">',
                    '<span class="tc-icon"><i class="fa-regular ' + statusIcon + '"></i></span>',
                    '<span class="tc-name">Test Case #' + (idx + 1) + (tc.name ? ': ' + tc.name : '') + '</span>',
                    '<span class="tc-weight">Weight: ' + (tc.weight || 1) + '</span>',
                    '</div>',
                    '<div class="tc-details">',
                    tc.input ? '<div class="tc-detail-row"><span class="tc-label">Input:</span><code class="tc-code">' + escHtml(tc.input) + '</code></div>' : '',
                    tc.expectedOutput ? '<div class="tc-detail-row"><span class="tc-label">Expected:</span><code class="tc-code">' + escHtml(tc.expectedOutput) + '</code></div>' : '',
                    tc.actualOutput ? '<div class="tc-detail-row"><span class="tc-label">Actual:</span><code class="tc-code">' + escHtml(tc.actualOutput) + '</code></div>' : '',
                    tc.error ? '<div class="tc-detail-row"><span class="tc-label">Error:</span><span class="tc-error-msg">' + escHtml(tc.error) + '</span></div>' : '',
                    '<div class="tc-metrics">',
                    tc.timeMs != null ? '<span class="tc-metric"><i class="fa-regular fa-clock"></i> ' + tc.timeMs + 'ms</span>' : '',
                    tc.memoryKb != null ? '<span class="tc-metric"><i class="fa-solid fa-memory"></i> ' + formatMemory(tc.memoryKb) + '</span>' : '',
                    '</div>',
                    '</div>',
                    '</div>'
                ].join('');
            });
            document.getElementById('test-pass-count').textContent = passed + ' passed';
            document.getElementById('test-fail-count').textContent = failed + ' failed';
            document.getElementById('test-total-count').textContent = testCases.length + ' total';
        } catch (err) {
    ...
    async function renderStyleReport(sid) {
        ...
        try {
            ...
            if (!report) return;
            section.style.display = 'block';
            document.getElementById('style-tool').textContent = 'Tool: ' + (report.tool || 'Unknown');
            const issues = report.issues || [];
            document.getElementById('style-issues').textContent = issues.length + ' issues';
            container.innerHTML = '';
            if (issues.length === 0) {
            ...
            issues.forEach(issue => {
                ...
                const sevIcon = sev === 'error' ? 'fa-circle-exclamation' : 'fa-triangle-exclamation';
                const sevClass = sev === 'error' ? 'style-issue-error' : 'style-issue-warning';
                container.innerHTML += [
                    '<div class="style-issue ' + sevClass + '">',
                    '<span class="style-issue-icon"><i class="fa-solid ' + sevIcon + '"></i></span>',
                    '<div class="style-issue-body">',
                    '<span class="style-issue-msg">' + escHtml(issue.message || issue.msg || '') + '</span>',
                    issue.line != null ? '<span class="style-issue-loc">Line ' + issue.line + (issue.column ? ', Col ' + issue.column : '') + '</span>' : '',
                    issue.ruleId ? '<span class="style-issue-rule">[' + escHtml(issue.ruleId) + ']</span>' : '',
                    '</div>',
                    '</div>'
                ].join('');
            });
    ...
    async function renderBenchmark(sid) {
        ...
        try {
            ...
            container.innerHTML = '';
            const metrics = [
                { label: 'Your Runtime', value: bench.runtimeMs, unit: 'ms', icon: 'fa-regular fa-clock', median: bench.medianRuntimeMs, better: 'lower' },
                { label: 'Your Memory', value: bench.memoryKb, unit: null, icon: 'fa-solid fa-memory', median: bench.medianMemoryKb, better: 'lower' },
                { label: 'Test Cases Passed', value: bench.passed + '/' + bench.total, unit: '', icon: 'fa-solid fa-vial', median: null },
                { label: 'Score', value: bench.score != null ? bench.score + '%' : '--', unit: '', icon: 'fa-solid fa-star', median: null },
            ];
            metrics.forEach(m => {
                let compareHtml = '';
                if (m.median != null && m.value != null && m.better) {
                    ...
                    compareHtml = '<div class="bench-compare ' + diffClass + '">' + diff + '% ' + betterText + ' class median (' + med + (m.unit || '') + ')</div>';
                }
    ...
    async function renderFeedback(sid) {
        ...
        try {
            ...
            const rubricContainer = document.getElementById('feedback-rubric-container');
            if (rubricContainer && feedbackData.rubricScores) {
                rubricContainer.innerHTML = '';
                feedbackData.rubricScores.forEach(rs => {
                    const pct = rs.maxPoints ? Math.round((rs.score / rs.maxPoints) * 100) : 0;
                    rubricContainer.innerHTML += [
                        '<div class="fb-rubric-item">',
                        '<div class="fb-rubric-top">',
                        '<span class="fb-rubric-name">' + escHtml(rs.criteria || rs.name || '') + '</span>',
                        '<span class="fb-rubric-score">' + rs.score + '/' + rs.maxPoints + ' (' + pct + '%)</span>',
                        '</div>',
                        '<div class="fb-rubric-bar"><div class="fb-rubric-fill" style="width:' + pct + '%"></div></div>',
                        '</div>'
                    ].join('');
                });
            }
            ...
            if (textEl && feedbackData.comment) textEl.textContent = feedbackData.comment;
            else if (textEl) textEl.textContent = 'No feedback yet.';

            const graderEl = document.getElementById('grader-name');
            if (graderEl) graderEl.textContent = feedbackData.grader || feedbackData.gradedBy || '--';
            const dateEl = document.getElementById('graded-date');
            if (dateEl) dateEl.textContent = feedbackData.date || feedbackData.gradedAt || '--';

            if (feedbackData.videoUrl) {
                const videoContainer = document.getElementById('feedback-video-container');
                ...
                if (prContainer && prList) {
                    ...
                    feedbackData.peerReviews.forEach(pr => {
                        prList.innerHTML += [
                            ...
                            '<div class="pr-header"><strong>' + escHtml(pr.reviewerName || 'Anonymous') + '</strong>',
                            pr.score != null ? '<span class="pr-score">Score: ' + pr.score + '/5</span>' : '',
                            '</div>',
                            pr.comment ? '<p class="pr-comment">' + escHtml(pr.comment) + '</p>' : '',
                            '</div>'
                        ].join('');
                    });
                }
    ...
    async function showAIAnalysis(sid) {
        ...
        try {
            let review;
            if (projectsClient && typeof projectsClient.getSubmissionReview === 'function') {
            ...
            if (review && (review.reasoning || review.evidence || typeof review.confidence_score === 'number')) {
                ...
                const confidencePercent = typeof review.confidence_score === 'number' ? Math.round(review.confidence_score * 100) : null;
                document.getElementById('ai-reasoning-text').textContent = review.reasoning || 'No semantic reasoning returned.';
                ...
                if (toggleBtn) {
                    toggleBtn.addEventListener('click', () => {
                        ...
                        toggleBtn.innerHTML = isHidden ? '<i class="fa-solid fa-eye-slash"></i> Hide AI Analysis' : '<i class="fa-solid fa-brain"></i> View AI Analysis';
                    });
                }
            }
        } catch (err) {
    ...
    async function loadAssignmentFromBackend(aid) {
        try {
            ...
            console.warn('[AssignmentContent] Backend fetch failed, using local data:', err);
        }
    }
    ...
    async function loadExistingResults(sid) {
        try {
            ...
            if (ev && ev.getResults) {
                const res = await ev.getResults(sid);
                const evalStatus = String(res?.data?.evaluationStatus || res?.evaluationStatus || res?.data?.status || '');
                if (evalStatus === 'completed' || evalStatus === 'passed' || evalStatus === 'approved') {
                    if (submitSection) submitSection.style.display = 'none';
                    if (pulseCard) pulseCard.classList.add('active');
                    Object.values(document.querySelectorAll('.pulse-step')).forEach(el => el.classList.add('step-done'));
                    renderTestResults(sid);
                    renderStyleReport(sid);
                    renderBenchmark(sid);
                    renderFeedback(sid);
            ...
            const storedStatus = localStorage.getItem(`last_sub_status_${data?.milestoneId}`);
            if (storedStatus && (storedStatus === 'completed' || storedStatus === 'passed' || storedStatus === 'approved')) {
                if (submitSection) submitSection.style.display = 'none';
            }
        } catch (err) {
    ...
    function populateUI(data) {
        ...
        if (data.instructions) {
            ...
            if (data.instructions.points) {
                data.instructions.points.forEach(pt => { instrHtml += '<li>' + pt + '</li>'; });
            }
            instrHtml += '</ul>';
            instrContainer.innerHTML = instrHtml;
        } else {
            instrContainer.innerHTML = '<p>' + (data.description || 'No instructions provided.') + '</p>';
        }
        ...
        if (data.files && data.files.length > 0) {
            ...
            data.files.forEach(file => {
                ...
                fileContainer.innerHTML += '<div class="file-item"><div class="file-info"><i class="' + icon + '"></i> ' + (file.name || 'file') + '</div><a href="#" class="download-link">Download</a></div>';
            });
        } else {
            fileContainer.innerHTML = '<div style="color: var(--text-secondary); font-size: 0.9rem;">No files attached.</div>';
        }
        ...
        if (data.rubric && data.rubric.length > 0) {
            ...
            data.rubric.forEach(item => {
                rubricContainer.innerHTML += '<div class="rubric-item"><span class="rubric-name">' + (item.criteria || item.name || '') + '</span><span class="rubric-percent">' + (item.percent || item.maxPoints || '') + '</span></div>';
            });
        } else {
            rubricContainer.innerHTML = '<div style="color: var(--text-secondary); font-size: 0.9rem;">No rubric specified.</div>';
        }

        if (data.feedback) {
            document.getElementById('feedback-text').textContent = data.feedback.comment || 'No feedback yet.';
            ...
            if (data.feedback.comment && data.feedback.comment !== 'No feedback yet.' && data.feedback.comment !== '...') {

(function () {
    ...
    function renderCourseCard(c) {
        ...
        if (recentSub && !isDraft) {
            ...
            if (recentSub.submittedAt) {
                try {
                    ...
                    stime = sh < 1 ? 'Just now' : sh < 24 ? sh + ' hours ago' : Math.floor(sh / 24) + ' days ago';
                } catch (_) {}
            }
            recentSubHtml = '<div class="inst-recent-sub">'
                + '<div class="inst-sub-icon"><i class="fa-solid fa-file-lines"></i></div>'
                + '<div class="inst-sub-text">'
                + '<p><strong>' + sname + '</strong> submitted</p>'
                + '<p class="inst-sub-title">' + stitle + '</p>'
                + '<span class="inst-sub-time">' + stime + '</span>'
                + '</div></div>';
        }

        if (isDraft) {
            return '<div class="inst-course-card inst-draft" data-id="' + cid + '" data-status="draft">'
                + '<div class="inst-card-top">'
                + '<h3>' + cname + ' <span class="inst-draft-badge">DRAFT</span></h3>'
                + '</div>'
        ...
        return '<div class="inst-course-card" data-id="' + cid + '" data-status="' + cstatus + '">'
            + '<div class="inst-card-top">'
            + '<h3>' + cname + '</h3>'
            + pendingBadge
            + '</div>'
            ...
            + '<button class="inst-action-btn inst-grade-btn" data-id="' + cid + '">Grade Work</button>'
            + '<button class="inst-settings-icon" data-id="' + cid + '"><i class="fa-solid fa-gear"></i></button>'
            + '</div></div>';
    }
    ...
    function renderCourses(courses) {
        ...
        if (!grid) return;
        if (!courses.length) {
            grid.innerHTML = '<div class="inst-empty-state"><p>No courses found for this filter.</p></div>';
            return;
        }
    ...
    function applyFilter(filter) {
        ...
        if (filter === 'all') {
            ...
            filtered = allCourses.filter(function (c) {
                ...
                if (filter === 'active') return status === 'active' || status === 'published' || status === 'in_progress';
        ...
        var activeBtn = document.querySelector('.inst-filter-btn[data-filter="' + filter + '"]');
    ...
    async function init() {
        ...
        try {
            ...
            try {
                ...
                var fallbackData = fallbackResp && (
                    (fallbackResp.data && fallbackResp.data.items) ||
                    fallbackResp.data || fallbackResp.courses || []
                );
            ...
            searchInput.addEventListener('input', function () {
                ...
                if (currentFilter !== 'all') {
                    filtered = filtered.filter(function (c) {
                        ...
                        if (currentFilter === 'active') return s === 'active' || s === 'published' || s === 'in_progress';

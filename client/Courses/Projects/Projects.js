...
function renderCacheIndicator(key) {
    ...
    return '<span class="cache-indicator ' + (isFresh ? 'cache-fresh' : 'cache-stale') + '" title="' + escapeHtml(label) + '">'
        + '<i class="fa-solid ' + (isFresh ? 'fa-database' : 'fa-triangle-exclamation') + '"></i> '
        + label
        + '</span>';
}
...
async function loadProjectsOverview(forceRefresh) {
    ...
    if (!forceRefresh) {
        ...
        if (cached) {
            ...
            setGroupWorkspaceStatus('ok', 'Student mode: Group Workspace is read-only. Showing cached data.');
            return;
        }
    }
    ...
    try {
        ...
        renderProjects();
        updateHeaderStats();
        setApiNoticeWithCache(payload.pageError || '', payload.pageError ? 'empty' : '', cacheKey);
        ...
        if (cached) {
            ...
            renderProjects();
            updateHeaderStats();
            setApiNoticeWithCache('Using cached data. Live update failed.', 'empty', cacheKey);
            setGroupWorkspaceStatus('error', 'Student mode: Group Workspace is read-only. Showing stale cached data.');
            return;
        }
...
function renderTeamPanel(project) {
    ...
    var apiId = project.apiProjectId || project._id || project.id;
    var manageBtn = '<button class="btn btn-outline btn-sm" onclick="openTeamManagementModal(\'' + escapeHtml(apiId) + '\')" style="margin-top:10px;width:100%;justify-content:center;"><i class="fa-solid fa-gear"></i> Manage Team</button>';
...
function renderGitHubPanel(project) {
    ...
    var repoHtml = repo
        ...
        : '<button class="github-connect-btn" onclick="openGithubConnectModal(\'' + escapeHtml(project.apiProjectId || project._id || project.id) + '\')"><i class="fa-brands fa-github"></i> Connect GitHub Repository</button>';
    ...
    var viewCommitsBtn = hasData
        ? '<button class="btn btn-outline btn-sm" onclick="openCommitHistoryModal(\'' + escapeHtml(project.apiProjectId || project._id || project.id) + '\')" style="margin-top:10px;width:100%;justify-content:center;"><i class="fa-solid fa-code-commit"></i> View Commit History</button>'
        : '';
    var analyticsBtn = hasData
        ? '<button class="btn btn-outline btn-sm" onclick="openContributionAnalyticsModal(\'' + escapeHtml(project.apiProjectId || project._id || project.id) + '\')" style="margin-top:6px;width:100%;justify-content:center;"><i class="fa-solid fa-chart-pie"></i> Contribution Analytics</button>'
        : '';
        ...
        try {
            ...
            if (['completed', 'passed', 'approved', 'failed', 'error'].includes(status)) {
                ...
                if (['failed', 'error'].includes(status)) {
                    document.getElementById('btn-retry-submission')?.classList.add('show');
                }
...
function renderPulseResultDetails(status, statusData) {
    ...
    const elapsed = projectsPageState.submissionStartTime
        ? Math.round((Date.now() - projectsPageState.submissionStartTime) / 1000) + 's'
        : '—';
    document.getElementById('pulse-result-time').textContent = elapsed;

    const isSuccess = ['completed', 'passed', 'approved'].includes(status);
    document.getElementById('pulse-result-status').textContent = isSuccess ? 'Completed' : 'Failed';
    document.getElementById('pulse-result-status').className = 'pulse-result-value ' + (isSuccess ? 'pulse-result-pass' : 'pulse-result-fail');

    const grade = statusData?.grade ?? statusData?.score ?? '—';
    document.getElementById('pulse-result-grade').textContent = grade !== '—' ? String(grade) + '/100' : '—';
    ...
    if (testResults.length > 0) {
        testResults.forEach(function (t) {
            ...
            item.innerHTML = '<i class="fa-solid fa-' + (passed ? 'circle-check" style="color:var(--status-graded-text)' : 'circle-xmark" style="color:var(--status-late-text)') + '"></i><span>' + escapeHtml(t.name || t.testName || 'Test') + '</span>';
...
function updatePulseUI(status) {
    const steps = { init: 'step-init', clone: 'step-clone', verify: 'step-verify', test: 'step-test', grade: 'step-grade' };
    Object.values(steps).forEach((id) => {
        const element = document.getElementById(id);
        if (element) element.classList.remove('step-active', 'step-done', 'step-failed');
    });
    ...
    if (['failed', 'error'].includes(status)) {
        Object.values(steps).forEach((id) => {
            ...
            if (el) { el.classList.add('step-done'); el.classList.remove('step-active'); }
        });
    }
}
...
function checkGithubAuthStatus() {
    ...
    statusEl.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Checking GitHub connection...';
    ...
    svc.githubService.getConfig().then(function (res) {
        var configured = res?.configured ?? res?.data?.configured ?? false;
        var appName = res?.appName || res?.data?.appName || '';
        var user = null;
        ...
        if (configured || ghUser) {
            ...
            document.getElementById('gh-authenticated-user').textContent = ghUser || appName || 'GitHub';
...
window.startGithubOAuth = function () {
    ...
    var svc = window.NibrasServices;
    if (svc && svc.githubService) {
        svc.githubService.getInstallUrl().then(function (res) {
...
function fallbackGithubOAuth(returnTo) {
    ...
    function tryOAuth(idx) {
        ...
        fetch(base + '/v1/github/config', { method: 'GET', headers: { Accept: 'application/json' } }).then(function (r) {
            if (r.status !== 404) {
                window.open(base + '/v1/github/oauth/start?return_to=' + returnTo, '_blank', 'width=800,height=700');
                pollGithubAuth();
            } else {
                tryOAuth(idx + 1);
            }
...
function pollGithubAuth() {
    var statusEl = document.getElementById('gh-auth-status');
    statusEl.style.display = '';
    statusEl.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Waiting for GitHub authorization...';
    ...
    var interval = setInterval(function () {
        ...
        if (!svc || !svc.githubService) {
            ...
            statusEl.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> GitHub service unavailable.';
        ...
        svc.githubService.getConfig().then(function (res) {
            ...
            if (configured) {
                ...
                document.getElementById('gh-authenticated-user').textContent = res?.appName || res?.data?.appName || 'GitHub';
                document.getElementById('gh-repo-step').style.display = '';
                setGithubStep(2);
            } else if (attempts >= maxAttempts) {
                clearInterval(interval);
                statusEl.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Authorization timed out. Please try again.';
                document.getElementById('gh-auth-actions').style.display = '';
            }
        }).catch(function () {
            if (attempts >= maxAttempts) {
                clearInterval(interval);
                statusEl.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Could not verify authorization. Please try again.';
                document.getElementById('gh-auth-actions').style.display = '';
            }
        });
    }, 2000);
...
window.confirmGithubConnect = function () {
    ...
    if (!ghPattern.test(repoUrl)) {
        errorEl.textContent = 'Please enter a valid GitHub repository URL (e.g. https://github.com/owner/repo).';
        errorEl.style.display = '';
        return;
    }
    ...
    validateEl.innerHTML = '<span style="color:var(--text-secondary);font-size:0.85rem;"><i class="fa-solid fa-circle-notch fa-spin"></i> Validating repository...</span>';

    var svc = window.NibrasServices;

    // Validate the repo
    var validatePromise = (svc && svc.githubService)
        ? svc.githubService.validateRepo(repoUrl).catch(function () { return null; })
        : Promise.resolve(null);

    validatePromise.then(function (validation) {
        if (validation) {
            validateEl.innerHTML = '<span style="color:var(--status-graded-text);font-size:0.85rem;"><i class="fa-solid fa-circle-check"></i> Repository validated: ' + escapeHtml(validation.owner + '/' + validation.name) + '</span>';
        } else {
            validateEl.innerHTML = '<span style="color:var(--text-secondary);font-size:0.85rem;"><i class="fa-solid fa-info-circle"></i> Repository URL accepted</span>';
        }

        // Connect the repo to the project
        var connectPromise;
        if (svc && svc.projectService && githubConnectProjectId) {
            connectPromise = svc.projectService.update(githubConnectProjectId, {
                repoUrl: repoUrl,
                repoBranch: branch,
        ...
        connectPromise.then(function () {
            ...
            connectBtn.innerHTML = '<i class="fa-solid fa-link"></i> Connect Repository';

            document.getElementById('gh-connect-step').style.display = 'none';
            document.getElementById('gh-success-state').style.display = '';
            document.getElementById('gh-connected-repo-name').textContent = repoUrl;
            setGithubStep(3);
            ...
            connectBtn.disabled = false;
            connectBtn.innerHTML = '<i class="fa-solid fa-link"></i> Connect Repository';
            errorEl.textContent = err?.message || 'Failed to connect repository. Please try again.';
            errorEl.style.display = '';
        });
    }).catch(function () {
        connectBtn.disabled = false;
        connectBtn.innerHTML = '<i class="fa-solid fa-link"></i> Connect Repository';
        errorEl.textContent = 'Could not validate repository URL. Please check and try again.';
        errorEl.style.display = '';
    });
};
...
document.addEventListener('input', function (e) {
    if (e.target && e.target.id === 'gh-repo-url') {
        ...
        if (connectStep && val) {
            ...
            document.getElementById('gh-connect-branch-display').textContent = document.getElementById('gh-repo-branch')?.value || 'main';
...
window.openCommitHistoryModal = function (projectId) {
    ...
    document.getElementById('commit-repo-name').textContent = repo || 'Local project';
...
function loadCommitHistory(projectId, project) {
    ...
    if (svc && svc.projectService && apiId) {
        svc.projectService.getById(apiId).then(function (res) {
            var data = res?.data || res;
...
function renderCommitHistory(commits, project) {
    ...
    commits.forEach(function (c) {
        ...
        if (!contributors[author]) {
            contributors[author] = { name: author, commits: 0, linesAdded: 0, linesRemoved: 0 };
        }
    ...
    var contribList = Object.keys(contributors).map(function (k) { return contributors[k]; });
    var totalCommits = commits.length;
    var totalLines = contribList.reduce(function (s, c) { return s + c.linesAdded + c.linesRemoved; }, 0);
    var dateRange = commits.length > 0
        ? (commits[commits.length - 1]?.date || commits[commits.length - 1]?.timestamp || '') + ' — ' + (commits[0]?.date || commits[0]?.timestamp || '')
        : '';

    // Summary bar
    document.getElementById('commit-summary').innerHTML = ''
        + '<div class="commit-summary-item"><strong>' + totalCommits + '</strong> commits</div>'
        + '<div class="commit-summary-item"><strong>' + contribList.length + '</strong> contributors</div>'
        + '<div class="commit-summary-item"><strong>' + totalLines + '</strong> lines changed</div>'
        + (dateRange ? '<div class="commit-summary-item" style="margin-left:auto;"><i class="fa-regular fa-calendar"></i> ' + escapeHtml(dateRange) + '</div>' : '');

    // Contributor breakdown with bars
    var maxCommits = Math.max.apply(null, contribList.map(function (c) { return c.commits; }));
    var contribHtml = '<div class="commit-contrib-title"><i class="fa-solid fa-users"></i> Contributors</div>';
    contribList.forEach(function (c) {
        var pct = totalCommits > 0 ? Math.round((c.commits / totalCommits) * 100) : 0;
        var color = 'var(--accent-blue)';
        contribHtml += '<div class="commit-contrib-row">'
            + '<span class="commit-contrib-name">' + escapeHtml(c.name) + '</span>'
            + '<div class="commit-contrib-track"><div class="commit-contrib-fill" style="width:' + pct + '%;background:' + color + ';"></div></div>'
            + '<span class="commit-contrib-pct">' + pct + '%</span>'
            + '<span class="commit-contrib-stat">+' + c.linesAdded + ' / -' + c.linesRemoved + '</span>'
            + '</div>';
    });
    document.getElementById('commit-contrib-breakdown').innerHTML = contribHtml;

    // Commit list
    var listHtml = '<div class="commit-contrib-title" style="margin-top:16px;"><i class="fa-solid fa-timeline"></i> Recent Commits</div>';
    commits.slice(0, 50).forEach(function (c) {
        ...
        listHtml += '<div class="commit-item">'
            + '<div class="commit-item-left">'
            + '<div class="commit-item-msg">' + escapeHtml(msg) + '</div>'
            + '<div class="commit-item-meta">'
            + '<span><i class="fa-regular fa-user"></i> ' + escapeHtml(author) + '</span>'
            + (formattedDate ? '<span><i class="fa-regular fa-clock"></i> ' + formattedDate + '</span>' : '')
            + '</div></div>'
            + (shortSha ? '<span class="commit-item-sha">' + shortSha + '</span>' : '')
            + '</div>';
    });
...
window.openContributionAnalyticsModal = function (projectId) {
    ...
    document.getElementById('analytics-project-name').textContent = (project && project.details && project.details.title) || 'Project';
...
function loadContributionAnalytics(projectId, project) {
    ...
    if (svc && svc.projectService && apiId) {
        svc.projectService.getById(apiId).then(function (res) {
            var data = res?.data || res;
            var commits = (data && Array.isArray(data.commits)) ? data.commits : (project ? project.commits || [] : []);
...
function renderContributionAnalytics(commits, contribution, project) {
    ...
    commits.forEach(function (c) {
        ...
        if (!memberStats[author]) {
            memberStats[author] = { name: author, commits: 0, linesAdded: 0, linesRemoved: 0, percentage: 0 };
        }
    ...
    var members = Object.keys(memberStats).map(function (k) { return memberStats[k]; });
    var totalCommits = members.reduce(function (s, m) { return s + m.commits; }, 0);
    var totalAdded = members.reduce(function (s, m) { return s + m.linesAdded; }, 0);
    var totalRemoved = members.reduce(function (s, m) { return s + m.linesRemoved; }, 0);
    var maxCommits = Math.max.apply(null, members.map(function (m) { return m.commits; })) || 1;
    var maxLines = Math.max.apply(null, members.map(function (m) { return m.linesAdded + m.linesRemoved; })) || 1;

    // Summary cards
    document.getElementById('analytics-summary').innerHTML = ''
        + '<div class="analytics-stat-card"><span class="analytics-stat-value">' + members.length + '</span><span class="analytics-stat-label">Contributors</span></div>'
        + '<div class="analytics-stat-card"><span class="analytics-stat-value">' + totalCommits + '</span><span class="analytics-stat-label">Total Commits</span></div>'
        + '<div class="analytics-stat-card"><span class="analytics-stat-value">+' + totalAdded + '/-' + totalRemoved + '</span><span class="analytics-stat-label">Lines Changed</span></div>';

    // --- Commits Tab ---
    var commitsChart = '';
    members.forEach(function (m) {
        var pct = Math.round((m.commits / totalCommits) * 100);
        var color = m.name === (project && project.teamMembers && project.teamMembers[0] && project.teamMembers[0].name) ? '#3b82f6' : '#6366f1';
        commitsChart += '<div class="analytics-bar-row">'
            + '<span class="analytics-bar-label">' + escapeHtml(m.name) + '</span>'
            + '<div class="analytics-bar-track"><div class="analytics-bar-fill" style="width:' + pct + '%;background:' + color + ';"></div></div>'
            + '<span class="analytics-bar-stat">' + m.commits + ' (' + pct + '%)</span>'
            + '</div>';
    });
    document.getElementById('analytics-commits-chart').innerHTML = commitsChart;

    var commitsTable = '<table><thead><tr><th>Member</th><th>Commits</th><th>Contribution %</th></tr></thead><tbody>';
    members.forEach(function (m) {
        var pct = totalCommits > 0 ? Math.round((m.commits / totalCommits) * 100) : 0;
        commitsTable += '<tr><td>' + escapeHtml(m.name) + '</td><td>' + m.commits + '</td><td>' + pct + '%</td></tr>';
    });
    ...
    members.forEach(function (m) {
        ...
        linesChart += '<div class="analytics-bar-row">'
            + '<span class="analytics-bar-label">' + escapeHtml(m.name) + '</span>'
            + '<div class="analytics-bar-track" style="height:24px;">'
            + '<div class="analytics-bar-fill" style="width:' + pct + '%;background:linear-gradient(90deg,#22c55e ' + addedPct + '%,#ef4444 0%);"></div>'
            + '</div>'
            + '<span class="analytics-bar-stat">+' + m.linesAdded + '/-' + m.linesRemoved + '</span>'
            + '</div>';
    });
    document.getElementById('analytics-lines-chart').innerHTML = linesChart;

    var linesTable = '<table><thead><tr><th>Member</th><th>Lines Added</th><th>Lines Removed</th><th>Net</th></tr></thead><tbody>';
    members.forEach(function (m) {
        var net = m.linesAdded - m.linesRemoved;
        var netStr = (net >= 0 ? '+' : '') + net;
        linesTable += '<tr><td>' + escapeHtml(m.name) + '</td><td style="color:var(--status-graded-text);">+' + m.linesAdded + '</td><td style="color:var(--status-late-text);">-' + m.linesRemoved + '</td><td>' + netStr + '</td></tr>';
    });
    ...
    commits.forEach(function (c) {
        ...
        if (!rawDate) return;
        var day;
        try { day = new Date(rawDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }); } catch (_) { return; }
    ...
    var maxFreq = Math.max.apply(null, freqDates.map(function (d) { return freqMap[d].count; })) || 1;

    var freqHtml = '';
    freqDates.forEach(function (day) {
        var f = freqMap[day];
        var pct = Math.round((f.count / maxFreq) * 100);
        var memberList = Object.keys(f.members).map(function (m) { return escapeHtml(m) + ' (' + f.members[m] + ')'; }).join(', ');
        freqHtml += '<div class="analytics-frequency-item">'
            + '<span class="analytics-frequency-date">' + day + '</span>'
            + '<div class="analytics-frequency-bar"><div class="analytics-frequency-fill" style="width:' + pct + '%;"></div></div>'
            + '<span class="analytics-frequency-count">' + f.count + '</span>'
            + '</div>';
    });
    document.getElementById('analytics-frequency-timeline').innerHTML = freqHtml
...
document.addEventListener('click', function (e) {
    ...
    document.querySelectorAll('.analytics-tab-panel').forEach(function (p) { p.classList.remove('active'); });
...
window.openTeamManagementModal = function (projectId) {
    ...
    document.getElementById('team-project-name').textContent = (project && project.details && project.details.title) || 'Project';
...
function loadTeamData(projectId, project) {
    ...
    teamMembers = (project && project.teamMembers) ? project.teamMembers.slice() : [];
...
function renderTeamMembersList() {
    ...
    if (!teamMembers.length) {
        container.innerHTML = '<p style="text-align:center;padding:20px;color:var(--text-secondary);font-size:0.85rem;">No members yet. Add team members using the invite form below.</p>';
        return;
    }

    container.innerHTML = teamMembers.map(function (m, idx) {
        var name = m.name || m.username || m.email || 'Member';
        var role = m.role || 'member';
        var initials = name.split(/\s+/).map(function (s) { return s[0]; }).join('').toUpperCase().slice(0, 2);
        var roleOptions = ['member', 'admin', 'reviewer'].map(function (r) {
            return '<option value="' + r + '"' + (r === role ? ' selected' : '') + '>' + r.charAt(0).toUpperCase() + r.slice(1) + '</option>';
        }).join('');

        return '<div class="team-member-card" data-idx="' + idx + '">'
            + '<div class="team-member-avatar-sm">' + escapeHtml(initials) + '</div>'
            + '<div class="team-member-card-info">'
            + '<div class="team-member-card-name">' + escapeHtml(name) + '</div>'
            + '</div>'
            + '<select class="team-member-role-select" data-idx="' + idx + '" onchange="updateTeamMemberRole(' + idx + ', this.value)">' + roleOptions + '</select>'
            + '<button class="team-member-remove-btn" onclick="removeTeamMember(' + idx + ')" title="Remove member"><i class="fa-solid fa-xmark"></i></button>'
            + '</div>';
    }).join('');
}
...
function inviteTeamMember() {
    ...
    var existing = teamMembers.some(function (m) {
        return (m.name || '').toLowerCase() === name.toLowerCase() || (m.email || '').toLowerCase() === name.toLowerCase();
    });
    if (existing) {
...
function saveTeamSettings() {
    ...
    var project = projectsPageState.ui.projects.find(function (e) {
        return e.apiProjectId === teamManagementProjectId || e.domId === IDs.toDom(teamManagementProjectId);
    });
    ...
    if (svc && svc.projectService && teamManagementProjectId) {
        savePromise = svc.projectService.update(teamManagementProjectId, {
            teamName: teamName,
    ...
    savePromise.then(function () {
        msgEl.style.display = '';
        msgEl.innerHTML = '<i class="fa-solid fa-circle-check"></i> Team saved successfully!';
        setTimeout(function () { msgEl.style.display = 'none'; }, 2500);
        renderProjectDetails();
    }).catch(function () {
        msgEl.style.display = '';
        msgEl.innerHTML = '<i class="fa-solid fa-circle-check"></i> Team saved locally!';
        setTimeout(function () { msgEl.style.display = 'none'; }, 2500);
        renderProjectDetails();
    });
}

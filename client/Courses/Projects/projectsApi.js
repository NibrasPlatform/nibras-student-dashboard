(function initProjectsApi(global) {
    ...
    function createClient(options = {}) {
        ...
        const settings = {
            ...
            getAuthToken: options.getAuthToken
                || (() => global.NibrasShared?.auth?.getToken?.() || global.NibrasApi?.getToken?.() || global.localStorage?.getItem('token') || null),
            fetchImpl,
        };

        return Object.freeze({
            getProjectsOverview: (request) => getProjectsOverview(request || {}, settings),
            getMilestoneFeedbackHistory: (request) => getMilestoneFeedbackHistory(request || {}, settings),
            submitMilestone: (payload) => submitMilestone(payload || {}, settings),
            getSubmissionStatus: (submissionId) => getSubmissionStatus(submissionId, settings),
            getSubmissionReview: (submissionId) => getSubmissionReview(submissionId, settings),
    ...
    async function submitMilestone(payload, settings) {
        ...
        return {
            ...
            data: {
                ...
                status: normalizeSubmissionStatus(response?.status || response?.data?.status || ''),
                raw: response,
            },
        };
    }
    ...
    async function requestJsonWithCompatibility(candidates, options, settings, behavior = {}) {
    ...
    async function requestJson(path, options, settings) {
        ...
        if (!response.ok) {
            throw createRequestError({
                message: parsed?.message || parsed?.error || defaultStatusMessage(response.status),
    ...
    function createRequestError({ message, code = 'REQUEST_FAILED', status = 0, payload = null, url = '', cause = null }) {
    ...
    function normalizeDashboardPayload(payload) {
        ...
        const milestonesByProject = isObject(payload?.milestonesByProject) ? payload.milestonesByProject : {};
        const statsByProject = isObject(payload?.statsByProject) ? payload.statsByProject : {};

        const normalizedProjects = projects.map((project, index) => {
            const projectId = String(project?.id || project?.projectId || `project-${index + 1}`);
            const milestoneList = Array.isArray(project?.milestones)
                ? project.milestones
                : (Array.isArray(milestonesByProject[projectId]) ? milestonesByProject[projectId] : []);

            const normalizedMilestones = milestoneList.map((milestone) => normalizeMilestone(milestone, projectId));
            const stats = normalizeProjectStats(statsByProject[projectId] || project?.stats, normalizedMilestones);
            const deliveryMode = String(project?.deliveryMode || project?.type || '').toLowerCase();
        ...
        const statusCounters = normalizedProjects.reduce((acc, project) => {
    ...
    function normalizeMilestone(milestone, projectId) {
        const status = normalizeMilestoneStatus(milestone?.status || milestone?.state || 'pending');
        ...
        return {
            ...
            dueLabel: String(milestone?.dueDateLabel || milestone?.dueLabel || formatDateLabel(milestone?.dueAt || milestone?.dueDate)),
    ...
    function normalizeProjectStats(stats, milestones) {
        ...
        const total = toInt(safeStats.total, toInt(safeStats.totalMilestones, milestones.length));
        const approved = toInt(safeStats.approved, toInt(safeStats.approvedCount, 0));
        const inReview = toInt(safeStats.in_review, toInt(safeStats.inReview, toInt(safeStats.submitted, toInt(safeStats.submittedCount, 0))));
        const complete = toInt(safeStats.complete, Math.min(total, approved + inReview));
        const completionRaw = toInt(safeStats.completion, toInt(safeStats.completionRate, total > 0 ? Math.round((approved / total) * 100) : 0));
    ...
    function normalizeFeedback(review, latestSubmission) {
        if (!review) {
            return {
                ...
                reviewerComment: latestSubmission?.reviewerComment || 'No feedback available yet.',
            };
        }

        return {
            status: normalizeMilestoneStatus(review?.status || latestSubmission?.status || 'pending'),
            reviewerComment: String(review?.comment || review?.summary || latestSubmission?.reviewerComment || 'No feedback available yet.'),
            score: typeof review?.score === 'number' ? review.score : null,
        };
    }

    function toTrackingSubmissionPayload(payload) {
        const submissionType = String(payload.submissionType || payload.submission_type || 'github').toLowerCase();
        const submissionValue = String(payload.resourceLink || payload.submissionValue || '').trim();
    ...
    function normalizeSubmissionStatus(status) {
        ...
        if (['running', 'verifying', 'processing', 'submitted', 'under_review', 'reviewing', 'needs_review'].includes(normalized)) return 'running';
        if (['approved', 'graded', 'passed', 'complete', 'completed'].includes(normalized)) return 'completed';
    ...
    function normalizeMilestoneStatus(status) {
        const normalized = String(status || '').trim().toLowerCase();
        if (['approved', 'graded', 'passed'].includes(normalized)) return 'approved';
        if (['complete', 'completed'].includes(normalized)) return 'complete';
        if (['changes_requested', 'needs_changes'].includes(normalized)) return 'needs_changes';
        if (['submitted', 'under_review', 'in_review', 'reviewing'].includes(normalized)) return 'in_review';

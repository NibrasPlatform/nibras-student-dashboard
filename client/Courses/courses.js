...
async function initCourses() {
    ...
    console.log('[COURSES.JS] Found container, rendering courses for level:', currentLevel);
    ...
    function renderPracticeCard(course) {
        ...
        var html = [
            ...
            course.features.map(function (f) { return '<li><i class="fa-solid fa-check"></i> ' + f + '</li>'; }).join(''),
            '</div>',
    ...
    async function hydrateCoursesFromAdmin(level) {
        ...
        try {
            ...
            var mappedCourses = adminCourses.filter(function (c) {
                return (c.adminCourseId || c.backendCourseId || c.remoteCourseId) && c.level === levelFilter;
            });
            ...
            if (coursesService && typeof coursesService.getProgress === 'function') {
                ...
                mappedCourses.forEach(function (c, i) {
                    ...
                    if (result.status === 'fulfilled' && result.value) {
                        var pct = result.value?.data?.percentage
                    ...
                    if (c.progress === undefined || c.progress === null || c.progress === 0) {
                        try {
                            var uid = '';
                            try { var u = JSON.parse(localStorage.getItem('user')); uid = u?._id || u?.id || ''; } catch (_) {}

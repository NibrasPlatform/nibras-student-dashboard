window.NibrasReact.run(() => {
    ...
    function getUserDisplay() {
        try {
            ...
            return {
                ...
                initials: (u.name || u.username || 'User').split(' ').map(function (n) { return n[0]; }).join('').substring(0, 2).toUpperCase(),
    ...
    function renderRooms() {
        ...
        if (!rooms || rooms.length === 0) {
            grid.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><i class="fa-solid fa-video"></i></div><h3>No study rooms yet</h3><p>Create a room to start studying with others in real-time</p></div>';
        ...
        rooms.forEach(function (room, idx) {
            ...
            card.innerHTML = [
                ...
                '<span class="room-card-name">' + escapeHtml(room.name || 'Untitled') + '</span>',
                '<span class="room-status-badge ' + (isLive ? 'live' : 'ended') + '">' + (isLive ? '<i class="fa-solid fa-circle"></i> Live' : 'Ended') + '</span>',
                '</div>',
                '<div class="room-card-desc">' + (shortDesc ? escapeHtml(shortDesc) : 'No description') + '</div>',
                '<div class="room-card-footer">',
                '<div class="participant-avatars">',
                renderParticipantAvatars(room.participants, participantCount),
                '<span class="participant-count-text">' + participantCount + ' ' + (participantCount === 1 ? 'participant' : 'participants') + '</span>',
                '</div>',
                '<button class="btn btn-primary btn-sm join-room-btn" data-room-id="' + encodeURIComponent(room._id || room.id) + '" ' + (isLive ? '' : 'disabled') + '>' + (isLive ? '<i class="fa-solid fa-video"></i> Join' : '<i class="fa-solid fa-lock"></i> Ended') + '</button>',
                '</div>',
            ].join('');
    ...
    function renderParticipantAvatars(participants, count) {
        ...
        if (Array.isArray(participants) && participants.length > 0) {
            ...
            for (var i = 0; i < maxShow; i++) {
                ...
                var initials = '?';
                if (p.name) {
                    initials = p.name.split(' ').map(function (n) { return n[0]; }).join('').substring(0, 2).toUpperCase();
                } else if (p.username) {
                    initials = p.username.substring(0, 2).toUpperCase();
                }
                html += '<span class="participant-avatar" title="' + escapeHtml(p.name || p.username || 'User') + '">' + escapeHtml(initials) + '</span>';
            }
            if (participants.length > 4) {
                html += '<span class="participant-avatar" style="background-color:var(--text-tertiary);">+' + (participants.length - 4) + '</span>';
            }
        } else if (count > 0) {
    ...
    async function loadRooms() {
        try {
            var data = await apiRequestWithFallback('/api/rooms', { method: 'GET', auth: true });
    ...
    document.getElementById('btn-create-room')?.addEventListener('click', async function () {
        ...
        if (!name) {
            if (errorEl) { errorEl.textContent = 'Please enter a room name.'; errorEl.style.display = 'block'; }
            return;
        }
        ...
        try {
            ...
            var result = await apiRequestWithFallback('/api/rooms', { method: 'POST', auth: true, body: body });
            var roomId = result?._id || result?.id || result?.room?._id || result?.room?.id;
            if (roomId) {
                window.location.href = './room.html?id=' + encodeURIComponent(roomId);
            } else {
                loadRooms();
            ...
            if (errorEl) { errorEl.textContent = 'Failed to create room: ' + (err.message || 'Unknown error'); errorEl.style.display = 'block'; }
            btn.disabled = false;
    ...
    document.getElementById('room-name')?.addEventListener('keydown', function (e) {
    ...
    function initSocket() {
        var baseUrl = (adminApiBase || trackingApiBase || 'https://nibras-backend.up.railway.app/api').replace(/\/api\/?$/, '').replace(/\/+$/, '');

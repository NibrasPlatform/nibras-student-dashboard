window.NibrasReact.run(function () {
    ...
    var resolveServiceUrl = window.NibrasApiConfig?.getServiceUrl?.bind(window.NibrasApiConfig)
        || window.NibrasShared?.resolveServiceUrl
        || function () { return null; };

    var adminApiBase = String(resolveServiceUrl('admin') || 'https://nibras-backend.up.railway.app/api').replace(/\/+$/, '');
    var trackingApiBase = String(resolveServiceUrl('tracking') || 'https://nibras-backend.up.railway.app/api').replace(/\/+$/, '');
    ...
    function buildServiceCandidates() {
        ...
        serviceCandidates.forEach(function (service) {
            ...
            if (!base) return;
            var dup = unique.some(function (e) { return e.base === base; });
    ...
    async function apiRequestWithFallback(path, options) {
        ...
        for (var i = 0; i < candidates.length; i++) {
            try {
                return await apiFetch(path, Object.assign({}, options, { service: candidates[i].service }));
            } catch (err) {
    ...
    function getSocketBaseUrl() {
        var base = (adminApiBase || trackingApiBase || 'https://nibras-backend.up.railway.app/api').replace(/\/api\/?$/, '').replace(/\/+$/, '');
        return base;
    }
    ...
    function initSocket() {
        ...
        if (!roomId) {
            document.getElementById('loading-video').innerHTML = '<div class="empty-state"><div class="empty-state-icon"><i class="fa-solid fa-circle-exclamation"></i></div><h3>No room ID specified</h3><p><a href="./rooms.html" style="color:var(--accent-blue);">Go back to rooms</a></p></div>';
            return;
        }
        ...
        script.onload = function () {
            ...
            socket.on('connect', function () {
                socket.emit('studyroom:join', { roomId: roomId, userId: userId, name: userName });
            });
            ...
            socket.on('signal:offer', function (data) {
                ...
                if (pc) {
                    pc.setRemoteDescription(new RTCSessionDescription(data.offer)).then(function () {
                        ...
                        socket.emit('signal:answer', { to: data.from, answer: pc.localDescription, roomId: roomId });
                    }).catch(function (err) {
            ...
            socket.on('signal:answer', function (data) {
                ...
                if (pc && pc.signalingState !== 'stable') {
                    pc.setRemoteDescription(new RTCSessionDescription(data.answer)).catch(function (err) {
            ...
            socket.on('signal:ice-candidate', function (data) {
                ...
                if (pc && data.candidate) {
                    pc.addIceCandidate(new RTCIceCandidate(data.candidate)).catch(function (err) {
            ...
            socket.on('studyroom:info', function (data) {
                ...
                if (metaEl) {
                    var parts = [];
                    if (data.participantCount !== undefined) parts.push(data.participantCount + ' participant' + (data.participantCount !== 1 ? 's' : ''));
    ...
    function createPeerConnection(peerId, initiator) {
        ...
        pc.onicecandidate = function (event) {
            if (event.candidate && socket) {
                socket.emit('signal:ice-candidate', { to: peerId, candidate: event.candidate, roomId: roomId });
            }
        };
        ...
        pc.ontrack = function (event) {
            if (event.streams && event.streams[0]) {
                ...
                addVideoContainer(peerId, remoteStream, participants[peerId]?.name || 'Peer');
            }
        };
        ...
        if (initiator) {
            pc.createOffer().then(function (offer) {
                ...
                if (socket) {
                    socket.emit('signal:offer', { to: peerId, offer: pc.localDescription, roomId: roomId });
                }
            }).catch(function (err) {
    ...
    async function startLocalMedia() {
        try {
            ...
            document.getElementById('loading-video').innerHTML = '<div class="empty-state"><div class="empty-state-icon"><i class="fa-solid fa-circle-exclamation"></i></div><h3>Could not access camera/microphone</h3><p>' + (err.message || 'Please check permissions') + '</p></div>';
        }
    }
    ...
    function addVideoContainer(id, stream, displayName) {
        ...
        container.innerHTML = [
            ...
            '</div>',
            '<video id="video-el-' + id + '" autoplay playsinline style="display:none;"></video>',
            '<div class="video-overlay">',
            '<span class="video-overlay-name">' + escapeHtml(name) + '</span>',
            '<span class="video-overlay-icons"><i class="fa-solid fa-volume-high" id="audio-indicator-' + id + '"></i></span>',
            '</div>',
        ].join('');
    ...
    function updateLocalMicIcon(enabled) {
        ...
        if (micIcon) {
            micIcon.className = enabled ? 'fa-solid fa-microphone' : 'fa-solid fa-microphone-slash';
        }
    }
    ...
    // Participant List UI
    // ============================================================
    participants[localUserId] = { id: localUserId, name: userName + ' (you)', isYou: true };
    ...
    function updateParticipantsUI() {
        ...
        var allParticipants = {};
        allParticipants[localUserId] = participants[localUserId] || { id: localUserId, name: userName + ' (you)', isYou: true };
        ...
        for (var pid in allParticipants) {
            ...
            item.innerHTML = [
                ...
                p.isYou ? '<span class="status-icon" style="font-size:0.7rem;color:var(--text-tertiary);">(you)</span>' : '',
            ].join('');
    ...
    function toggleMic() {
        ...
        if (btn) {
            ...
            btn.innerHTML = isMicEnabled ? '<i class="fa-solid fa-microphone"></i>' : '<i class="fa-solid fa-microphone-slash"></i>';
    ...
    function toggleCam() {
        ...
        if (btn) {
            ...
            btn.innerHTML = isCamEnabled ? '<i class="fa-solid fa-video"></i>' : '<i class="fa-solid fa-video-slash"></i>';
    ...
    async function startLocalCameraTrack() {
        try {
            var newStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
    ...
    function uploadRecording() {
        ...
        var uploadUrl = (adminApiBase || trackingApiBase || '').replace(/\/api\/?$/, '') + '/api/rooms/' + encodeURIComponent(roomId) + '/recordings';

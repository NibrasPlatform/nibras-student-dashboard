window.NibrasReact.run(function () {
    ...
    var platformStatsContainer = document.getElementById('platform-stats-container');
    ...
    function renderContestEngagement(data) {
        ...
        if (!data) {
            contestContainer.innerHTML = '<p style="color:var(--text-secondary);padding:1rem;">No contest data available.</p>';
            return;
        }
    ...
    function renderPlatformStats(platformData) {
        ...
        if (!platformData) {
            platformStatsContainer.innerHTML = '<p style="color:var(--text-secondary);padding:1rem;">Platform data unavailable (Phase 9 API not ready).</p>';
            return;
        }

        var activeUsers = platformData.activeUsers || {};
        var totalQuestions = platformData.questionsAsked || platformData.totalQuestions || platformData.questions || 0;
        var totalAnswers = platformData.answersProvided || platformData.totalAnswers || platformData.answers || 0;
        var contestsHeld = platformData.contestsHeld || platformData.totalContests || platformData.contests || 0;
        var totalSubs = platformData.totalSubmissions || platformData.submissions || 0;

        var dailyUsers = activeUsers.daily || activeUsers.dau || platformData.dailyActiveUsers || 0;
        var weeklyUsers = activeUsers.weekly || activeUsers.wau || platformData.weeklyActiveUsers || 0;
        var monthlyUsers = activeUsers.monthly || activeUsers.mau || platformData.monthlyActiveUsers || 0;

        platformStatsContainer.innerHTML = '';
        var platformStats = [
            { label: 'Daily Active Users', value: String(dailyUsers), icon: 'fa-solid fa-users' },
            { label: 'Weekly Active Users', value: String(weeklyUsers), icon: 'fa-solid fa-users-gear' },
            { label: 'Monthly Active Users', value: String(monthlyUsers), icon: 'fa-solid fa-users-rays' },
            { label: 'Total Questions', value: String(totalQuestions), icon: 'fa-solid fa-question-circle' },
            { label: 'Answers Given', value: String(totalAnswers), icon: 'fa-solid fa-reply-all' },
            { label: 'Contests Held', value: String(contestsHeld), icon: 'fa-solid fa-trophy' },
        ];
        if (totalSubs > 0) {
            platformStats.push({ label: 'Total Submissions', value: String(totalSubs), icon: 'fa-regular fa-paper-plane' });
        }
    ...
    function renderCommunityMetrics(platformData, studentPerfData) {
        ...
        var questions = (studentPerfData && studentPerfData.studentStats && studentPerfData.studentStats.questionsAsked) || (platformData && platformData.questionsAsked) || '—';
        var answers = (studentPerfData && studentPerfData.studentStats && studentPerfData.studentStats.answersProvided) || (platformData && platformData.answersProvided) || '—';
        var communityScore = (studentPerfData && studentPerfData.studentStats && studentPerfData.studentStats.reputation) || (platformData && platformData.communityScore) || '—';
        var votesCast = (platformData && (platformData.votesCast || platformData.votes)) || null;
        var activeUsers = (platformData && platformData.activeUsers && platformData.activeUsers.total) || null;

        commContainer.innerHTML = '';
        commContainer.innerHTML += '<div class="metric-row"><span class="metric-label">Questions Asked</span><span class="metric-val">' + questions + '</span></div>';
        commContainer.innerHTML += '<div class="metric-row"><span class="metric-label">Answers Provided</span><span class="metric-val">' + answers + '</span></div>';
        if (votesCast !== null) {
            commContainer.innerHTML += '<div class="metric-row"><span class="metric-label">Votes Cast</span><span class="metric-val">' + votesCast + '</span></div>';
        }
        if (activeUsers !== null) {
            commContainer.innerHTML += '<div class="metric-row"><span class="metric-label">Active Users</span><span class="metric-val">' + activeUsers + '</span></div>';
        }
        var scoreClass = communityScore !== '—' && communityScore > 0 ? 'metric-green' : '';
        commContainer.innerHTML += '<div class="metric-row"><span class="metric-label">Community Score</span><span class="metric-val ' + scoreClass + '">' + communityScore + '</span></div>';
    }
    ...
    function renderEngagementTrendChart(trendData) {
        ...
        var hasData = trendData && ((trendData.trends && trendData.trends.length > 0) || (Array.isArray(trendData) && trendData.length > 0));
        if (!hasData) {
            var wrapper = canvas.parentElement;
            wrapper.innerHTML = '<div class="chart-empty"><i class="fa-solid fa-chart-line"></i><span>Engagement trends will appear once the backend aggregates platform activity</span></div>';
            return;
        }

        var points = trendData.trends || trendData;
        var labels = points.map(function (p) { return p.period || p.date || p.label || ''; });
        var userData = points.map(function (p) { return p.activeUsers || p.users || p.dau || 0; });
        var questionData = points.map(function (p) { return p.questions || p.questionsAsked || 0; });
        var answerData = points.map(function (p) { return p.answers || p.answersProvided || 0; });
        ...
        if (questionData.some(function (v) { return v > 0; })) {
            datasets.push({
                label: 'Questions',
                data: questionData,
                borderColor: '#f59e0b',
        ...
        if (answerData.some(function (v) { return v > 0; })) {
            datasets.push({
                label: 'Answers',
                data: answerData,
                borderColor: '#10b981',
    ...
    if (user && user._id && services && services.backendAnalyticsService) {
        var studentPerfPromise = services.backendAnalyticsService.getStudentPerformance(user._id).then(function (res) {
        ...
        var platformPromise = services.backendAnalyticsService.getPlatformMetrics().then(function (res) {
            var data = res && (res.data || res);
            return data;
        }).catch(function () {
            return null;
        ...
        services.backendAnalyticsService.getPlatformEngagement().then(function (res) {

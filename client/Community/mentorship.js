window.NibrasReact.run(function () {
    ...
    function loadSuggestions() {
        if (!services || !services.mentorshipService) {
            suggestionsContainer.innerHTML = '<div class="empty-state"><i class="fa-solid fa-user-group"></i><p>Mentorship service not available.</p></div>';
            return;
        }

        suggestionsContainer.innerHTML = '<div class="loading-state"><i class="fa-solid fa-spinner fa-spin" style="font-size:2rem;margin-bottom:1rem;display:block;"></i><p>Finding mentors for you...</p></div>';

        services.mentorshipService.getSuggestions(20).then(function (res) {
            var data = res && (res.data || res);
            var suggestions = Array.isArray(data) ? data : (Array.isArray(data.suggestions) ? data.suggestions : []);
            if (suggestions.length === 0) {
                suggestionsContainer.innerHTML = '<div class="empty-state"><i class="fa-solid fa-user-group"></i><p>No mentor suggestions available right now. Check back later!</p></div>';
                return;
            }
            renderSuggestions(suggestions);
        }).catch(function (err) {
            console.error('Failed to load mentor suggestions:', err);
            suggestionsContainer.innerHTML = '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Could not load mentor suggestions. Try again later.</p></div>';
        });
    }
    ...
    function renderSuggestions(suggestions) {
        ...
        suggestions.forEach(function (s) {
            ...
            var name = s.mentorName || s.name || s.fullName || s.mentor?.name || 'Mentor';
            ...
            if (typeof topics === 'string') topics = topics.split(',').map(function (t) { return t.trim(); });
            ...
            html += '<div class="mentor-card" data-mentor-id="' + esc(mentorId) + '">';
            ...
            if (bio) html += '<p style="font-size:0.88rem;color:var(--text-secondary);margin:0 0 0.75rem;">' + esc(bio.length > 120 ? bio.substring(0, 120) + '...' : bio) + '</p>';
            if (topics.length > 0) {
                html += '<div class="mentor-tags">';
                topics.forEach(function (t) {
                    html += '<span class="mentor-tag">' + esc(typeof t === 'string' ? t : (t.name || t)) + '</span>';
                });
                html += '</div>';
            }
            html += '<div class="mentor-stats">';
            if (confidence) html += '<span><i class="fa-solid fa-chart-line"></i> ' + esc(confidence) + (typeof confidence === 'number' || /^\d+$/.test(confidence) ? '% match' : '') + '</span>';
            if (responseTime) html += '<span><i class="fa-regular fa-clock"></i> ' + esc(responseTime) + '</span>';
            html += '</div>';
            html += '<div class="mentor-actions">';
            html += '<button class="btn-request' + (isRequested ? ' requested' : '') + '" data-mentor-id="' + esc(mentorId) + '" data-mentor-name="' + esc(name) + '"' + (isRequested ? ' disabled' : '') + '>' + (isRequested ? '<i class="fa-regular fa-circle-check"></i> Requested' : '<i class="fa-solid fa-user-plus"></i> Request Mentor') + '</button>';
            html += '</div>';
            html += '</div>';
        });
        html += '</div>';
    ...
    function loadMyRequests() {
        ...
        var items = [];
        try {
            var cached = JSON.parse(localStorage.getItem('mentorship_requests') || '[]');
        ...
        items.forEach(function (req) {
            ...
            if (message) html += '<span>' + esc(message.length > 80 ? message.substring(0, 80) + '...' : message) + '</span>';
            html += '</div>';
            html += '<div style="display:flex;align-items:center;gap:12px;">';
            if (createdAt) html += '<span style="font-size:0.8rem;color:var(--text-secondary);">' + formatDate(createdAt) + '</span>';
            html += '<span class="status-badge ' + esc(status) + '">' + esc(status.charAt(0).toUpperCase() + status.slice(1)) + '</span>';
            html += '</div>';
            html += '</div>';
        });
    ...
    submitBtn.addEventListener('click', function () {
        ...
        services.mentorshipService.requestMentor(currentMentorId, message).then(function () {
            ...
            var btn = suggestionsContainer.querySelector('.btn-request[data-mentor-id="' + esc(currentMentorId) + '"]');
            if (btn) {
                btn.disabled = true;
                btn.classList.add('requested');
                btn.innerHTML = '<i class="fa-regular fa-circle-check"></i> Requested';
            }

            try {
                var existing = JSON.parse(localStorage.getItem('mentorship_requests') || '[]');
                existing.unshift({
                    mentorId: currentMentorId,
                    mentorName: mentorNameDisplay.textContent.replace('Send a message to ', ''),
                    status: 'pending',
                    message: message,

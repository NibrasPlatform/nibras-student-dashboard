(function runAiTutorRecommendationPage(initializer) {
    ...
    const TRACK_DESCRIPTIONS = Object.freeze({
        ...
        'Information Track': 'Focus on data systems, analytics, and information management.',
        'Visual Computing': 'Explore computer graphics, vision, and image understanding.',
        ...
        'Computer Engineering': 'Blend hardware and software engineering practices.',
        'Human-Computer Interaction': 'Design user-centered systems with strong UX outcomes.',
        'Computational Biology': 'Apply computation and data science to biological domains.',
    });
    ...
    const normalizeGradeToPercent = (value) => {
        ...
        if (LETTER_GRADE_TO_PERCENT[upper] != null) return LETTER_GRADE_TO_PERCENT[upper];
    ...
    const toCourseCanonicalCode = (value) => {
        ...
        if (COURSE_ALIAS_TO_CODE[normalized]) return COURSE_ALIAS_TO_CODE[normalized];

        const match = raw.match(/(CS\s*\d+[A-Z]?|MATH\s*\d+[A-Z]?|ENGR\s*\d+[A-Z]?|PHIL\s*\d+[A-Z]?|BIO|CHEM)/i);
        if (!match) return null;
        const compact = String(match[1]).toUpperCase().replace(/[^A-Z0-9]/g, '');
    ...
    const formatExplanationToHtml = (rawText) => {
        ...
        const flushList = () => {
            ...
            htmlParts.push(`<ul class="rec-explanation-list">${listItems.join('')}</ul>`);
            listItems = [];
        };
    ...
    const toPriority = (index) => {
        ...
        if (index === 1) return { label: 'Medium Priority', className: 'badge-medium' };
    ...
    const explanationContainer = document.getElementById('rec-explanation-content');
    ...
    const loadBackendGrades = async () => {
        ...
        throw new Error('No compatible grade records were found from available backend endpoints.');
    };
    ...
    const renderRecommendations = (result, gradesCount) => {
        ...
        recContainer.innerHTML = recommendations.map((track, index) => {
            const priority = toPriority(index);
            const safeTrack = String(track || '').trim() || `Track ${index + 1}`;
            const reason = extractTrackReason(safeTrack, explanation) || TRACK_DESCRIPTIONS[safeTrack] || 'Recommended from your current profile.';
            const strengthChips = strengths.length
                ? strengths.map((entry) => `<span class="rec-strength-chip">${escapeHtml(entry)}</span>`).join('')
        ...
        try {
            ...
            if (sourceNote) {
                if (/route not found/i.test(message) || /recommend/i.test(message)) {
                    sourceNote.textContent = 'Recommendation API request failed. Configure a recommendation base URL (not a full /api/recommend path).';
                } else {
                    sourceNote.textContent = 'Recommendation data could not be generated from backend grades.';
                }
            }
            if (explanationContainer) {

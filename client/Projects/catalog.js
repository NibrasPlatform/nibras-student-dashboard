var roleState = { step: 1, card: null };
...
window.NibrasReact.run(function () {
    ...
    ['role-motivation', 'role-availability'].forEach(function (id) {
        ...
        el.addEventListener('input', function () {
            var countId = id === 'role-motivation' ? 'motivation-count' : 'availability-count';
...
function openRoleModal(card) {
    ...
    pills.forEach(function (p) {
        var text = p.textContent.replace(/\d+/g, '').trim();
...
function roleStep(dir) {
    if (dir === 'next' && roleState.step === 1) {
        var s1 = document.getElementById('role-choice-1').value;
        if (!s1) { alert('Please select your first choice role.'); return; }
    }

(function () {
    ...
    function setupGradeUI() {
        document.getElementById('grade-project-select').addEventListener('change', function () {
            var val = this.value;
        ...
        document.querySelectorAll('#grade-detail-modal .detail-tab').forEach(function (tab) {
            tab.addEventListener('click', function () {
                document.querySelectorAll('#grade-detail-modal .detail-tab').forEach(function (t) { t.classList.remove('active'); });
                this.classList.add('active');
                document.querySelectorAll('#grade-detail-modal .detail-panel').forEach(function (p) { p.classList.remove('active'); });
                var panel = document.getElementById('grade-panel-' + this.dataset.tab);
                if (panel) panel.classList.add('active');
            });
        });

        document.getElementById('grade-btn-submit-grade').addEventListener('click', submitGrade);

        document.getElementById('grade-detail-modal').addEventListener('click', function (e) {
    ...
    function loadGradeProjects() {
        ...
        if (!cid) {
            select.innerHTML = '<option value="">Select a course on Projects tab first.</option>';
            return;
        }
        document.getElementById('grade-course-subtitle').textContent = 'Course loaded';

        if (S && S.projectService) {
            S.projectService.listByCourse(cid).then(function (res) {
                var items = res?.data || [];
                if (!Array.isArray(items)) items = [];
                gradeProjects = items;
                populateGradeSelect(select, items);
                if (!items.length) {
                    document.getElementById('grade-empty-state').innerHTML = '<i class="fa-solid fa-diagram-project"></i><p>No projects yet. Create one in the Projects tab first.</p>';
                    document.getElementById('grade-empty-state').style.display = 'block';
                }
            }).catch(function () {
    ...
    function generateDemoGradeProjects() {
        return [
            { _id: 'demo-p1', title: 'Capstone Project', points: 200, teamSize: 3, milestones: [
                { name: 'Proposal', weight: 10, dueDate: new Date(Date.now() - 14 * 86400000).toISOString() },
                { name: 'Design Document', weight: 20, dueDate: new Date(Date.now() - 7 * 86400000).toISOString() },
                { name: 'Implementation', weight: 40, dueDate: new Date(Date.now() + 7 * 86400000).toISOString() },
                { name: 'Final Report', weight: 30, dueDate: new Date(Date.now() + 21 * 86400000).toISOString() },
            ]},
            { _id: 'demo-p2', title: 'Data Analysis Project', points: 150, teamSize: 2, milestones: [
                { name: 'Data Collection', weight: 25, dueDate: new Date(Date.now() - 3 * 86400000).toISOString() },
                { name: 'Analysis', weight: 50, dueDate: new Date(Date.now() + 14 * 86400000).toISOString() },
                { name: 'Presentation', weight: 25, dueDate: new Date(Date.now() + 28 * 86400000).toISOString() },
            ]},
        ];
    }
    ...
    function populateGradeSelect(select, items) {
        ...
        items.forEach(function (p) {
            ...
            opt.textContent = (p.title || 'Untitled') + ' (' + (p.points || '—') + ' pts)';
            select.appendChild(opt);
        });
    }

    function loadGradeSubmissions(projectId) {
        gradeCurrentProject = gradeProjects.find(function (p) { return (p._id || p.id) === projectId; });
        var tbody = document.getElementById('grade-submissions-body');
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:32px">Loading submissions...</td></tr>';

        if (S && S.projectService) {
            S.projectService.getSubmissions(projectId).then(function (res) {
                var items = res?.data || [];
                if (!Array.isArray(items)) items = [];
    ...
    function generateDemoGradeSubmissions(projectId) {
        var demoTeams = [
            { name: 'Team Alpha', members: ['Ahmed Hassan', 'Mariam Khalid', 'Youssef Ibrahim'] },
            { name: 'Team Beta', members: ['Laila Mostafa', 'Omar Abdelrahman'] },
            { name: 'Team Gamma', members: ['Nour El-Din', 'Hana Youssef', 'Karim Samir'] },
        ];
        ...
        gradeSubmissions = demoTeams.map(function (team, idx) {
            ...
            var msStatuses = milestones.map(function (ms) {
                ...
                return { name: ms.name, weight: ms.weight, completed: done, score: score, maxScore: 100 };
            });
            ...
            return {
                ...
                status: status,
                score: status === 'graded' ? Math.round(weightedScore * (p.points || 100) / 100) : status === 'evaluated' ? Math.round(weightedScore * (p.points || 100) / 100) : null,
                maxScore: p.points || 100,
                milestoneStatuses: msStatuses,
                contribution: team.members.map(function (m) {
                    return { name: m, commits: Math.floor(Math.random() * 30 + 5), linesAdded: Math.floor(Math.random() * 1000 + 100), linesRemoved: Math.floor(Math.random() * 500 + 50), percentage: 0 };
                }),
                githubRepo: 'https://github.com/nibras/' + team.name.toLowerCase().replace(/\s+/g, '-'),
                commits: [
                    { message: 'Initial project setup', author: team.members[0], date: new Date(Date.now() - 20 * 86400000).toISOString() },
                    { message: 'Implement core algorithm', author: team.members[1], date: new Date(Date.now() - 12 * 86400000).toISOString() },
                    { message: 'Add unit tests', author: team.members[0], date: new Date(Date.now() - 8 * 86400000).toISOString() },
                    { message: 'Fix edge cases in parser', author: team.members[2] || team.members[1], date: new Date(Date.now() - 5 * 86400000).toISOString() },
                    { message: 'Update documentation', author: team.members[1], date: new Date(Date.now() - 2 * 86400000).toISOString() },
                ],
                submittedAt: new Date(Date.now() - Math.random() * 10 * 86400000).toISOString(),
            };
        });

        gradeSubmissions.forEach(function (s) {
            var total = s.contribution.reduce(function (sum, c) { return sum + c.commits; }, 0);
    ...
    function renderGradeSubmissions() {
        ...
        var graded = gradeSubmissions.filter(function (s) { return s.status === 'graded'; }).length;
        var pending = gradeSubmissions.filter(function (s) { return s.status === 'pending'; }).length;
        ...
        document.getElementById('stat-pending').textContent = pending;
        document.getElementById('stat-avg').textContent = scores.length ? avg + '/' + Math.round(gradeSubmissions[0]?.maxScore || 100) : '--';

        tbody.innerHTML = gradeSubmissions.map(function (s, idx) {
            ...
            var completedMs = (s.milestoneStatuses || []).filter(function (m) { return m.completed; }).length;
            var totalMs = (s.milestoneStatuses || []).length;
            return '<tr>'
                + '<td class="team-name">' + escapeHtml(s.teamName || 'Unknown Team') + '</td>'
                + '<td class="members-cell">' + (s.members || []).join(', ') + '</td>'
                + '<td class="milestone-cell">' + completedMs + '/' + totalMs + '</td>'
                + '<td><span class="status-badge ' + statusClass + '">' + statusLabel + '</span></td>'
                + '<td class="score-cell">' + scoreDisplay + '</td>'
                + '<td><button class="grade-action-btn" data-g-idx="' + idx + '">' + (s.status === 'graded' ? 'Review' : 'Grade') + '</button></td>'
                + '</tr>';
        }).join('');
    ...
    function openGradeSubmission(idx) {
        ...
        document.getElementById('detail-team-name').textContent = escapeHtml(gradeCurrentSubmission.teamName || 'Team') + '\'s Project';
        ...
        document.querySelectorAll('#grade-detail-modal .detail-tab').forEach(function (t) { t.classList.remove('active'); });
        document.querySelector('#grade-detail-modal .detail-tab[data-tab="milestones"]').classList.add('active');
        document.querySelectorAll('#grade-detail-modal .detail-panel').forEach(function (p) { p.classList.remove('active'); });
        document.getElementById('grade-panel-milestones').classList.add('active');
    }

    function renderGradeMilestones(msStatuses) {
        var completed = msStatuses.filter(function (m) { return m.completed; }).length;
        ...
        msStatuses.forEach(function (ms) {
            ...
            container.innerHTML += '<div class="milestone-item ' + cls + '">'
                + '<span class="ms-icon"><i class="fa-regular ' + icon + '"></i></span>'
                + '<div class="ms-info"><strong>' + escapeHtml(ms.name || 'Milestone') + '</strong>'
                + '<span class="ms-weight">Weight: ' + (ms.weight || 0) + '%</span></div>'
                + '<span class="ms-score">' + scoreDisplay + '</span>'
                + '</div>';
        });
    }
    ...
    function renderGradeContribution(contribution) {
        if (!contribution || !contribution.length) {
            document.getElementById('grade-contribution-chart').innerHTML = '<p class="empty-hint">No contribution data available.</p>';
        ...
        contribution.forEach(function (c) {
            chartHtml += '<div class="contrib-bar-row">'
                + '<span class="contrib-name">' + escapeHtml(c.name) + '</span>'
                + '<div class="contrib-bar-track"><div class="contrib-bar-fill" style="width:' + c.percentage + '%"></div></div>'
                + '<span class="contrib-pct">' + c.percentage + '%</span>'
                + '</div>';
        });
        chartHtml += '</div>';
        document.getElementById('grade-contribution-chart').innerHTML = chartHtml;

        var tableHtml = '<table class="contrib-table"><thead><tr><th>Member</th><th>Commits</th><th>Lines Added</th><th>Lines Removed</th><th>Est. Contribution</th></tr></thead><tbody>';
        contribution.forEach(function (c) {
            tableHtml += '<tr><td>' + escapeHtml(c.name) + '</td><td>' + c.commits + '</td><td>' + c.linesAdded + '</td><td>' + c.linesRemoved + '</td><td>' + c.percentage + '%</td></tr>';
        });
        ...
        contribution.forEach(function (c, idx) {
            adjustHtml += '<div class="adjust-row"><label>' + escapeHtml(c.name) + '</label>'
                + '<input type="range" class="adjust-slider" data-g-idx="' + idx + '" min="0" max="100" value="' + c.percentage + '">'
                + '<input type="number" class="adjust-input" data-g-idx="' + idx + '" min="0" max="100" value="' + c.percentage + '" style="width:60px">'
                + '<span class="adjust-pct">%</span></div>';
        });
        document.getElementById('grade-adjustment-inputs').innerHTML = adjustHtml;

        document.querySelectorAll('#grade-adjustment-inputs .adjust-slider').forEach(function (slider) {
            slider.addEventListener('input', function () {
                var idx = this.getAttribute('data-g-idx');
                var input = document.querySelector('.adjust-input[data-g-idx="' + idx + '"]');
                if (input) input.value = this.value;
            });
        });
        document.querySelectorAll('#grade-adjustment-inputs .adjust-input').forEach(function (input) {
            input.addEventListener('input', function () {
                var idx = this.getAttribute('data-g-idx');
                var slider = document.querySelector('.adjust-slider[data-g-idx="' + idx + '"]');
    ...
    function renderGradeTeam(submission) {
        ...
        submission.members.forEach(function (member) {
            container.innerHTML += '<div class="team-member-chip"><i class="fa-solid fa-user"></i> ' + escapeHtml(member) + '</div>';
        });

        document.getElementById('grade-team-github-repo').innerHTML = submission.githubRepo
            ? '<a href="' + escapeHtml(submission.githubRepo) + '" target="_blank"><i class="fa-brands fa-github"></i> ' + escapeHtml(submission.githubRepo) + '</a>'
            : 'No repo connected';
        ...
        commitContainer.innerHTML = commits.map(function (c) {
            return '<div class="commit-item">'
                + '<div class="commit-message"><i class="fa-solid fa-code-commit"></i> ' + escapeHtml(c.message) + '</div>'
                + '<div class="commit-meta"><span>' + escapeHtml(c.author) + '</span>'
                + '<span>' + (c.date ? new Date(c.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '') + '</span></div>'
                + '</div>';
        }).join('');
    }
    ...
    function renderGradeForm(submission) {
        ...
        if (!msStatuses.length) {
            container.innerHTML = '<p class="empty-hint">No milestones defined for this project.</p>';
            return;
        }
        ...
        msStatuses.forEach(function (ms) {
            ...
            row.innerHTML = '<div class="weighted-top">'
                + '<span class="weighted-name">' + escapeHtml(ms.name || 'Milestone') + '</span>'
                + '<span class="weighted-pct">Weight: ' + (ms.weight || 0) + '%</span>'
                + '</div>'
                + '<div class="weighted-input-row">'
                + '<label>Score</label>'
                + '<input type="number" class="weighted-score-input ms-score-input" data-weight="' + (ms.weight || 0) + '" data-max="' + max + '" value="' + score + '" min="0" max="' + max + '" step="1">'
                + '<span class="weighted-max">/ ' + max + '</span>'
                + '<span class="weighted-contribution">Weighted: <strong>' + weighted.toFixed(1) + '</strong></span>'
                + '</div>';
        ...
        document.getElementById('grade-feedback-text').value = submission.feedbackText || '';
    }
    ...
    function updateGradeTotal() {
        var inputs = document.querySelectorAll('#grade-weighted-score-breakdown .ms-score-input');
        var totalWeighted = 0;
        var maxWeighted = 0;
        ...
        document.getElementById('grade-grade-total').textContent = 'Overall: ' + totalWeighted.toFixed(1) + ' / ' + maxWeighted.toFixed(1) + ' (' + pct + '%)';
    }
    ...
    function submitGrade() {
        ...
        var msInputs = document.querySelectorAll('#grade-weighted-score-breakdown .ms-score-input');
        var milestoneScores = [];
        msInputs.forEach(function (input) {
            milestoneScores.push(parseFloat(input.value) || 0);
        });

        var feedbackText = document.getElementById('grade-feedback-text').value.trim();

        var adjustments = {};
        document.querySelectorAll('#grade-adjustment-inputs .adjust-input').forEach(function (input) {
            var idx = parseInt(input.getAttribute('data-g-idx'));
            var name = gradeCurrentSubmission.contribution && gradeCurrentSubmission.contribution[idx] ? gradeCurrentSubmission.contribution[idx].name : 'Member ' + idx;
            adjustments[name] = parseFloat(input.value) || 0;
        });

        var totalScore = milestoneScores.reduce(function (a, b) { return a + b; }, 0);

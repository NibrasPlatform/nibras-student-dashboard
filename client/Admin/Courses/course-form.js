window.NibrasReact.run(() => {
    ...
    function addSectionRow(data) {
        ...
        div.innerHTML =
            '<div class="form-group">' +
                ...
                '<input type="text" class="section-name" placeholder="e.g., Section A" value="' + (data.name ? data.name.replace(/"/g, '&quot;') : '') + '" required>' +
            '</div>' +
            '<div class="form-group">' +
                '<label>Capacity</label>' +
                '<input type="number" class="section-capacity" placeholder="Max students" min="1" value="' + (data.capacity || '') + '">' +
            '</div>' +
            '<div class="form-group">' +
                '<label>Schedule</label>' +
                '<input type="text" class="section-schedule" placeholder="e.g., Mon/Wed 10:00-11:30" value="' + (data.schedule ? data.schedule.replace(/"/g, '&quot;') : '') + '">' +
            '</div>' +
            '<button type="button" class="btn-remove-section" title="Remove section"><i class="fa-solid fa-xmark"></i></button>';

        div.querySelector('.btn-remove-section').addEventListener('click', function () {
            div.remove();
        });
    ...
    function collectFormData() {
        return {
            ...
            instructors: document.getElementById('input-instructors')?.value?.split(',').map(function (s) { return s.trim(); }).filter(Boolean),
            thumbnail: document.getElementById('input-thumbnail')?.value?.trim() || undefined,
    ...
    function redirectWithToast(url, message, type) {
        try {
            localStorage.setItem('adminToast', JSON.stringify({ message: message, type: type || 'success' }));
        } catch (_) {}
    ...
    function initForm() {
        ...
        if (courseId) {
            ...
            document.getElementById('form-subtitle').textContent = 'Update course details and sections';
    ...
    function fillForm(course) {
        ...
        if (course.instructors) {
            var instructors = Array.isArray(course.instructors)
                ? course.instructors.map(function (i) { return typeof i === 'string' ? i : (i.email || i.name || i._id || ''); }).join(', ')
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            if (isEditMode) {
                ...
                redirectWithToast('./courses.html', 'Course updated successfully', 'success');
            } else {
                res = await apiCall('/admin/courses', {
                    method: 'POST',
                    body: data,
                });
                redirectWithToast('./courses.html', 'Course created successfully', 'success');
            }
        } catch (error) {
            hideLoading();
            var submitBtn = document.getElementById('submit-btn');
            submitBtn.disabled = false;
            alert('Failed to ' + (isEditMode ? 'update' : 'create') + ' course: ' + (error.message || 'Unknown error'));
        }
    }

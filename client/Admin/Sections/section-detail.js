window.NibrasReact.run(() => {
    ...
    function getInitials(name) {
        if (!name) return 'S';
        return name.split(' ').map(function (s) { return s.charAt(0); }).join('').toUpperCase().slice(0, 2) || 'S';
    }
    ...
    async function apiCall(path, options) {
        ...
        if (window.NibrasShared?.apiFetch) return await window.NibrasShared.apiFetch(path, Object.assign({ service: 'admin' }, options));
    ...
    function initTheme() {
        ...
        if (btn) {
            btn.addEventListener('click', function () {
                ...
                var h = document.documentElement; var c = h.getAttribute('data-theme'); var n = c === 'light' ? 'dark' : 'light';
    ...
    async function loadSectionData() {
        ...
        if (!courseId || !sectionId) {
            headerContainer.innerHTML = '<div class="error-state" style="text-align:center;padding:2rem;"><i class="fa-solid fa-triangle-exclamation"></i><p>Missing course or section ID in URL.</p><a href="../Courses/courses.html" class="btn-primary" style="display:inline-flex;margin-top:0.5rem;">Back to Courses</a></div>';
        ...
        try {
            ...
            sectionData = sections.find(function (s) { return (s._id || s.id) === sectionId; });

            if (!sectionData) {
                headerContainer.innerHTML = '<div class="error-state" style="text-align:center;padding:2rem;"><i class="fa-solid fa-triangle-exclamation"></i><p>Section not found.</p><a href="../Courses/courses.html" class="btn-primary" style="display:inline-flex;margin-top:0.5rem;">Back to Courses</a></div>';
                infoContainer.innerHTML = '';
                studentsContainer.innerHTML = '';
                return;
            }
            ...
            headerContainer.innerHTML = '<div class="error-state" style="text-align:center;padding:2rem;"><i class="fa-solid fa-circle-exclamation" style="color:#ef4444;"></i><p>Failed to load section: ' + escapeHtml(error.message || 'Unknown error') + '</p><a href="../Courses/courses.html" class="btn-primary" style="display:inline-flex;margin-top:0.5rem;">Back to Courses</a></div>';
            infoContainer.innerHTML = '';
            studentsContainer.innerHTML = '';
        }
    }
    ...
    function renderHeader(course, section) {
        ...
        container.innerHTML =
            '<div class="section-header">' +
                '<div>' +
                    '<h1>' + escapeHtml(section.name || 'Unnamed Section') + '</h1>' +
                    '<span class="enrollment-badge">' + students.length + ' / ' + capacity + ' enrolled</span>' +
                '</div>' +
                '<div style="display:flex;gap:8px;">' +
                    '<a href="../Courses/course-form.html?id=' + encodeURIComponent(courseId || '') + '" class="btn-outline"><i class="fa-solid fa-pen"></i> Edit Course</a>' +
                '</div>' +
            '</div>';
    }
    ...
    function renderInfo(course, section) {
        ...
        container.innerHTML =
            '<div class="info-grid">' +
                '<div class="info-item"><label>Course</label><span>' + escapeHtml(course.title || course.name || '—') + '</span></div>' +
                '<div class="info-item"><label>Section Name</label><span>' + escapeHtml(section.name || '—') + '</span></div>' +
                '<div class="info-item"><label>Schedule</label><span>' + escapeHtml(section.schedule || 'Not set') + '</span></div>' +
                '<div class="info-item"><label>Capacity</label><span>' + (section.capacity || 'Unlimited') + '</span></div>' +
            '</div>';
    }
    ...
    function renderStudents(section) {
        ...
        if (students.length === 0) {
            ...
            if (emptyBtn) emptyBtn.addEventListener('click', function () { openBatchModal(); });
            return;
        }

        var html = '<table class="student-table"><thead><tr><th>#</th><th>Student</th><th>Email</th><th>Enrolled</th><th>Actions</th></tr></thead><tbody>';
        students.forEach(function (s, i) {
            ...
            html += '<tr>' +
                '<td>' + (i + 1) + '</td>' +
                '<td><div class="student-name-cell"><span class="student-avatar-sm">' + initials + '</span>' + escapeHtml(name) + '</div></td>' +
                '<td style="color:var(--text-secondary);font-size:0.82rem;">' + escapeHtml(email) + '</td>' +
                '<td style="font-size:0.82rem;color:var(--text-secondary);">' + enrolled + '</td>' +
                '<td><button class="btn-remove-student" data-id="' + encodeURIComponent(sid) + '" data-name="' + escapeHtml(name) + '"><i class="fa-solid fa-user-minus"></i> Remove</button></td>' +
            '</tr>';
        });
        html += '</tbody></table>';
        ...
        container.querySelectorAll('.btn-remove-student').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('remove-student-name').textContent = 'Remove "' + btn.dataset.name + '" from this section? This action can be undone by enrolling them again.';
    ...
    function initRemoveModal() {
        ...
        document.getElementById('remove-cancel').addEventListener('click', function () {
            modal.classList.remove('active'); removeTargetId = null;
        });
        modal.addEventListener('click', function (e) { if (e.target === modal) { modal.classList.remove('active'); removeTargetId = null; } });
        document.getElementById('remove-confirm').addEventListener('click', async function () {
            ...
            try {
                await apiCall('/admin/sections/' + encodeURIComponent(sectionId) + '/students/' + encodeURIComponent(removeTargetId), { method: 'DELETE' });
                showToast('Student removed from section', 'success');
                modal.classList.remove('active'); removeTargetId = null;
                loadSectionData();
            } catch (error) {
                showToast('Failed to remove: ' + (error.message || 'Unknown error'), 'error');
            } finally {
                btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-user-minus"></i> Remove Student';
            }
        });
    }
    ...
    function initBatchModal() {
        ...
        document.getElementById('batch-cancel').addEventListener('click', function () {
            modal.classList.remove('active'); batchStudents = [];
            document.getElementById('preview-section').style.display = 'none';
            document.getElementById('batch-confirm-btn').disabled = true;
        });
        ...
        document.getElementById('batch-preview-btn').addEventListener('click', function () {
            ...
            if (tab === 'manual') {
                ...
                if (!text) { alert('Please enter student emails or IDs.'); return; }
                students = text.split('\n').map(function (s) { return s.trim(); }).filter(Boolean);
                students = [...new Set(students)];
            } else if (tab === 'csv') {
                var fileInput = document.getElementById('csv-file-input');
                if (!fileInput.files || !fileInput.files[0]) { alert('Please upload a CSV file.'); return; }
                alert('CSV parsing is done on the backend. Please use the manual entry tab to enter student IDs.');
                return;
            }

            if (students.length === 0) { alert('No valid student identifiers found.'); return; }
        ...
        document.getElementById('batch-confirm-btn').addEventListener('click', async function () {
            ...
            try {
                await apiCall('/admin/sections/' + encodeURIComponent(sectionId) + '/enroll', {
                    method: 'POST',
                    body: { students: batchStudents },
                });
                showToast(batchStudents.length + ' student' + (batchStudents.length !== 1 ? 's' : '') + ' enrolled successfully', 'success');
                modal.classList.remove('active');
                batchStudents = [];
                document.getElementById('preview-section').style.display = 'none';
                loadSectionData();
            } catch (error) {
                showToast('Failed to enroll: ' + (error.message || 'Unknown error'), 'error');
            } finally {
                btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-user-plus"></i> Enroll Students';
            }
        });

        document.getElementById('batch-enroll-btn').addEventListener('click', function () { openBatchModal(); });

        var csvUpload = document.getElementById('csv-upload-area');
        var csvInput = document.getElementById('csv-file-input');
        csvUpload.addEventListener('click', function () { csvInput.click(); });
        csvInput.addEventListener('change', function () {
            if (csvInput.files && csvInput.files[0]) {
                csvUpload.querySelector('p').textContent = 'File selected: ' + csvInput.files[0].name;
            }
        });
    }

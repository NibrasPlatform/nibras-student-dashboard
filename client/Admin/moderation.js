window.NibrasReact.run(() => {
    ...
    async function loadQueue(status) {
        container.innerHTML = '<div class="loading-state" style="text-align:center;padding:3rem;color:var(--text-secondary);"><i class="fa-solid fa-spinner fa-spin" style="font-size:2rem;margin-bottom:1rem;display:block;"></i><p>Loading moderation queue...</p></div>';

        try {
            const flagService = window.NibrasServices?.flagService;
            if (!flagService) {
                container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Flag service not available. Make sure you are signed in as admin.</p></div>';
                return;
            }
            ...
            if (items.length === 0) {
                container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-check-circle"></i><p>All clear! No items in the queue.</p></div>';
                return;
            }
            ...
            items.forEach(function (flag) {
                ...
                const reporterName = flag.reporter?.name || flag.reporter || 'Anonymous';
                const targetPreview = flag.contentPreview || flag.targetPreview || (flag.targetBody ? flag.targetBody.substring(0, 200) : 'No preview available');
    ...
    container.addEventListener('click', async function (e) {
        ...
        try {
            ...
            if (action === 'dismiss') {
                ...
                if (!confirm('Ban the user who posted this? This action cannot be undone.')) {
                    btn.disabled = false;
                    btn.style.opacity = '';
                    return;
                }

(function () {
    ...
    function loadData() {
        ...
        if (!backendId) {
            document.getElementById('grades-body').innerHTML = '<tr><td colspan="6" style="text-align:center;padding:48px 16px;color:var(--text-secondary)">No course ID available.</td></tr>';
            return;
        }

        var assignmentPromise;
        if (S && S.assignmentAdminService) {
            assignmentPromise = S.assignmentAdminService.listByCourse(backendId).then(function (res) {
    ...
    function buildStudentData() {
        ...
        var enrolled = [];
        if (selectedCourse && selectedCourse.overview && selectedCourse.overview.stats) {
            var count = parseInt(selectedCourse.overview.stats.enrolled) || 5;
            for (var i = 1; i <= Math.min(count, 10); i++) {
                enrolled.push({ name: 'Student ' + i, email: 'student' + i + '@nibras.edu' });
            }
        }
        if (!enrolled.length) {
            enrolled = [
    ...
    function renderGrades() {
        ...
        if (!studentsData.length) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:48px;color:var(--text-secondary)">No student data available.</td></tr>';
            return;
        }
    ...
    function renderStats() {
        ...
        if (studentsData.length) {
            var avg = Math.round(studentsData.reduce(function (sum, s) { return sum + s.percentage; }, 0) / studentsData.length);
            document.getElementById('stat-avg').textContent = avg + '%';
            var passed = studentsData.filter(function (s) { return s.percentage >= 60; }).length;
            document.getElementById('stat-pass').textContent = Math.round((passed / studentsData.length) * 100) + '%';
        }
    }

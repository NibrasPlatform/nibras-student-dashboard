(function () {
    ...
    function init() {
        selectedCourse = window.NibrasCourses && typeof window.NibrasCourses.getSelectedCourse === 'function'
            ? window.NibrasCourses.getSelectedCourse() : null;
        if (!selectedCourse && courseId && window.NibrasCourses && typeof window.NibrasCourses.getCourseById === 'function') {
    ...
    function setupUI() {
        ...
        document.getElementById('btn-submit-grade').addEventListener('click', submitGrade);
    ...
    function loadProjects() {
        ...
        if (S && S.projectService) {
            S.projectService.listByCourse(backendId).then(function (res) {
                ...
                if (!items.length) {
                    document.getElementById('empty-state').innerHTML = '<i class="fa-solid fa-diagram-project"></i><p>No projects yet. Create one in the Project Builder first.</p>';
                    document.getElementById('empty-state').style.display = 'block';
                }
            }).catch(function () {
    ...
    function loadSubmissions(projectId) {
        currentProject = projects.find(function (p) { return (p._id || p.id) === projectId; });
    ...
    function renderSubmissions() {
        ...
        var scores = submissions.filter(function (s) { return s.score != null; }).map(function (s) { return s.score; });
        var avg = scores.length ? Math.round(scores.reduce(function (a, b) { return a + b; }, 0) / scores.length) : 0;

        document.getElementById('stat-total').textContent = total;
        document.getElementById('stat-graded').textContent = graded;
        document.getElementById('stat-pending').textContent = pending;
        document.getElementById('stat-avg').textContent = scores.length ? avg + '/' + Math.round(submissions[0]?.maxScore || 100) : '--';

        tbody.innerHTML = submissions.map(function (s, idx) {
            ...
            return '<tr>'
                ...
                + '<td><button class="grade-action-btn" data-idx="' + idx + '">' + (s.status === 'graded' ? 'Review' : 'Grade') + '</button></td>'
                + '</tr>';
        }).join('');
    ...
    function openSubmission(idx) {
        ...
        document.getElementById('detail-team-name').textContent = escapeHtml(currentSubmission.teamName || 'Team') + '\'s Project';
        ...
        document.querySelectorAll('.detail-tab').forEach(function (t) { t.classList.remove('active'); });
        document.querySelector('.detail-tab[data-tab="milestones"]').classList.add('active');
        document.querySelectorAll('.detail-panel').forEach(function (p) { p.classList.remove('active'); });
    ...
    function renderContribution(contribution) {
        if (!contribution || !contribution.length) {
            document.getElementById('contribution-chart').innerHTML = '<p class="empty-hint">No contribution data available.</p>';
        ...
        contribution.forEach(function (c, idx) {
            adjustHtml += '<div class="adjust-row"><label>' + escapeHtml(c.name) + '</label>'
                + '<input type="range" class="adjust-slider" data-idx="' + idx + '" min="0" max="100" value="' + c.percentage + '">'
                + '<input type="number" class="adjust-input" data-idx="' + idx + '" min="0" max="100" value="' + c.percentage + '" style="width:60px">'
        ...
        document.querySelectorAll('.adjust-slider').forEach(function (slider) {
            slider.addEventListener('input', function () {
                ...
                var input = document.querySelector('.adjust-input[data-idx="' + idx + '"]');
        ...
        document.querySelectorAll('.adjust-input').forEach(function (input) {
            input.addEventListener('input', function () {
                var idx = this.getAttribute('data-idx');
                var slider = document.querySelector('.adjust-slider[data-idx="' + idx + '"]');
    ...
    function renderTeam(submission) {
        ...
        document.getElementById('team-github-repo').innerHTML = submission.githubRepo
    ...
    function updateGradeTotal() {
        ...
        document.getElementById('grade-total').textContent = 'Overall: ' + totalWeighted.toFixed(1) + ' / ' + maxWeighted.toFixed(1) + ' (' + pct + '%)';
    }
    ...
    function submitGrade() {
        ...
        document.querySelectorAll('.adjust-input').forEach(function (input) {
            ...
            var name = currentSubmission.contribution && currentSubmission.contribution[idx] ? currentSubmission.contribution[idx].name : 'Member ' + idx;

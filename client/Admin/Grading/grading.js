(function () {
    ...
    function fixNavLinks() {
        ...
        if (navAssign && courseId) navAssign.href = '../AssignmentBuilder/assignment-builder.html?courseId=' + encodeURIComponent(courseId);
    ...
    function setupUI() {
        ...
        document.getElementById('btn-assign-peer').addEventListener('click', assignPeerReview);
    ...
    function loadAssignments() {
        ...
        if (S && S.assignmentAdminService) {
            S.assignmentAdminService.listByCourse(backendId).then(function (res) {
                var items = res?.data || [];
                if (!Array.isArray(items)) items = [];
                assignments = items;
                populateSelect(select, items);
                // Fallback: use local assignment data
                if (!items.length && selectedCourse && selectedCourse.assignments && selectedCourse.assignments.items) {
                    assignments = selectedCourse.assignments.items;
                    populateSelect(select, assignments);
                }
                if (!items.length) {
                    document.getElementById('empty-state').innerHTML = '<i class="fa-solid fa-file-circle-plus"></i><p>No assignments yet. Create one in the Assignment Builder first.</p>';
                    document.getElementById('empty-state').style.display = 'block';
                }
            }).catch(function () {
    ...
    function populateSelect(select, items) {
        ...
        items.forEach(function (a) {
            ...
            opt.textContent = (a.title || 'Untitled') + ' (' + (a.points || '—') + ' pts)';
    ...
    function loadSubmissions(assignmentId) {
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:32px">Loading submissions...</td></tr>';

        var backendId = selectedCourse?.adminCourseId || selectedCourse?.backendCourseId || courseId;

        if (S && S.evaluationService && S.evaluationService.listByAssignment) {
            S.evaluationService.listByAssignment(assignmentId).then(function (res) {
                var items = res?.data || [];
                if (!Array.isArray(items)) items = [];
                submissions = items;
    ...
    function generateDemoSubmissions(assignmentId) {
        var demoStudents = [
            'Ahmed Hassan', 'Mariam Khalid', 'Youssef Ibrahim',
            'Laila Mostafa', 'Omar Abdelrahman', 'Nour El-Din',
            'Hana Youssef', 'Karim Samir'
        ];
        var statuses = ['pending', 'evaluated', 'graded'];
        submissions = [];
        var a = assignments.find(function (x) { return (x._id || x.id || x.title) === assignmentId; });
        var maxPts = a ? (a.points || a.maxScore || 100) : 100;

        demoStudents.forEach(function (name, idx) {
            var status = statuses[idx % 3];
            var score = status === 'graded' ? Math.round(Math.random() * maxPts * 0.8 + maxPts * 0.2) : null;
    ...
    function generateDemoTestCases(status) {
        ...
        var tests = [
            { name: 'Basic positive', input: '[1, 2, 3]', expected: '6', passed: Math.random() > 0.2, timeMs: Math.round(Math.random() * 50 + 5), memoryKb: Math.round(Math.random() * 4096 + 1024) },
            { name: 'Mixed values', input: '[-2, 1, -3, 4, -1, 2, 1, -5, 4]', expected: '6', passed: Math.random() > 0.3, timeMs: Math.round(Math.random() * 80 + 10), memoryKb: Math.round(Math.random() * 4096 + 1024) },
            { name: 'All negative', input: '[-5, -2, -8, -1]', expected: '-1', passed: Math.random() > 0.1, timeMs: Math.round(Math.random() * 30 + 2), memoryKb: Math.round(Math.random() * 2048 + 512) },
            { name: 'Edge case (hidden)', input: '[100]', expected: '100', passed: Math.random() > 0.3, timeMs: Math.round(Math.random() * 20 + 1), memoryKb: Math.round(Math.random() * 1024 + 256), isHidden: true },
        ];
        return tests;
    }
    ...
    function generateDemoStyleIssues() {
        return [
            { message: 'Missing docstring for module', line: 1, severity: 'warning', ruleId: 'missing-module-docstring' },
            { message: 'Line too long (92 > 79 characters)', line: 8, severity: 'warning', ruleId: 'line-too-long' },
            { message: 'Variable name "arr" is too short', line: 4, severity: 'warning', ruleId: 'invalid-name' },
        ];
    }
    ...
    function renderSubmissions() {
        ...
        tbody.innerHTML = submissions.map(function (s, idx) {
            ...
            var dateDisplay = s.submittedAt ? new Date(s.submittedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
            return '<tr>'
                + '<td class="student-name">' + escapeHtml(s.studentName) + '</td>'
    ...
    function openSubmission(idx) {
        ...
        document.getElementById('detail-student-name').textContent = escapeHtml(currentSubmission.studentName) + '\'s Submission';
        document.getElementById('detail-language').textContent = currentSubmission.language || 'python';

        // Code
        document.getElementById('code-viewer').innerHTML = '<code>' + escapeHtml(currentSubmission.code || '// No code available') + '</code>';
        ...
        // Grade form
        renderGradeForm(currentSubmission.rubric || currentRubric, currentSubmission);
        ...
        // Activate first tab
        document.querySelectorAll('.detail-tab').forEach(function (t) { t.classList.remove('active'); });
        document.querySelector('.detail-tab[data-tab="code"]').classList.add('active');
        document.querySelectorAll('.detail-panel').forEach(function (p) { p.classList.remove('active'); });
        document.getElementById('panel-code').classList.add('active');
    }

    function renderDetailTests(testCases) {
        var passed = testCases.filter(function (t) { return t.passed; }).length;
        var failed = testCases.length - passed;
        document.getElementById('detail-test-pass').textContent = passed + ' passed';
        document.getElementById('detail-test-fail').textContent = failed + ' failed';
        document.getElementById('detail-test-total').textContent = testCases.length + ' total';

        var container = document.getElementById('detail-test-cases');
        container.innerHTML = '';
        if (!testCases.length) {
            container.innerHTML = '<p class="empty-hint">No test results available.</p>';
            return;
        }
        testCases.forEach(function (tc) {
            var cls = tc.passed ? 'tc-pass' : 'tc-fail';
            var icon = tc.passed ? 'fa-circle-check' : 'fa-circle-xmark';
            container.innerHTML += '<div class="test-case ' + cls + '">'
                + '<div class="tc-header"><span class="tc-icon"><i class="fa-regular ' + icon + '"></i></span>'
                + '<span class="tc-name">' + escapeHtml(tc.name || 'Test Case') + '</span>'
                + (tc.isHidden ? '<span class="tc-hidden-badge">Hidden</span>' : '')
                + '</div>'
                + '<div class="tc-details">'
                + (tc.input ? '<div class="tc-detail-row"><span class="tc-label">Input:</span><code>' + escapeHtml(tc.input) + '</code></div>' : '')
                + (tc.expected ? '<div class="tc-detail-row"><span class="tc-label">Expected:</span><code>' + escapeHtml(tc.expected) + '</code></div>' : '')
                + (tc.actualOutput ? '<div class="tc-detail-row"><span class="tc-label">Actual:</span><code>' + escapeHtml(tc.actualOutput) + '</code></div>' : '')
                + '<div class="tc-metrics">'
                + (tc.timeMs != null ? '<span><i class="fa-regular fa-clock"></i> ' + tc.timeMs + 'ms</span>' : '')
                + (tc.memoryKb != null ? '<span><i class="fa-solid fa-memory"></i> ' + tc.memoryKb + 'KB</span>' : '')
                + '</div></div></div>';
        });
    }
    ...
    function renderDetailStyle(issues) {
        ...
        if (!issues || !issues.length) {
            container.innerHTML = '<div class="style-clean"><i class="fa-solid fa-circle-check"></i> No style issues found!</div>';
            return;
        }
        issues.forEach(function (issue) {
            var sev = issue.severity || 'warning';
            var sevIcon = sev === 'error' ? 'fa-circle-exclamation' : 'fa-triangle-exclamation';
            var sevClass = sev === 'error' ? 'style-issue-error' : 'style-issue-warning';
            container.innerHTML += '<div class="style-issue ' + sevClass + '">'
                + '<span class="style-issue-icon"><i class="fa-solid ' + sevIcon + '"></i></span>'
                + '<div class="style-issue-body">'
                + '<span class="style-issue-msg">' + escapeHtml(issue.message || '') + '</span>'
                + (issue.line != null ? '<span class="style-issue-loc">Line ' + issue.line + '</span>' : '')
                + (issue.ruleId ? '<span class="style-issue-rule">[' + escapeHtml(issue.ruleId) + ']</span>' : '')
                + '</div></div>';
        });
    }
    ...
    function renderGradeForm(rubric, submission) {
        ...
        if (!rubric || !rubric.length) {
            container.innerHTML = '<p class="empty-hint">No rubric defined for this assignment.</p>';
            return;
        }

        rubric.forEach(function (criterion, idx) {
            var existingScore = submission.rubricScores ? (submission.rubricScores[idx] || 0) : 0;
            var maxPts = criterion.maxPoints || criterion.maxScore || 10;
            var div = document.createElement('div');
            div.className = 'rubric-score-row';
            div.innerHTML = '<div class="rubric-score-top">'
                + '<span class="rubric-score-name">' + escapeHtml(criterion.name || 'Criterion ' + (idx + 1)) + '</span>'
                + '<span class="rubric-score-max">/ ' + maxPts + '</span>'
                + '</div>'
                + '<input type="number" class="rubric-score-input" data-idx="' + idx + '" data-max="' + maxPts + '" value="' + existingScore + '" min="0" max="' + maxPts + '" step="0.5">';
        ...
        document.getElementById('grade-video-url').value = submission.videoUrl || '';
    }
    ...
    function updateGradeTotal() {
        ...
        document.getElementById('grade-total').textContent = 'Total: ' + total + ' / ' + maxTotal + ' (' + pct + '%)';
    }
    ...
    function renderPeerReviews(submission) {
        ...
        if (!reviews.length) {
            container.innerHTML = '<p class="empty-hint">No peer reviews assigned yet.</p>';
            return;
        }
        container.innerHTML = reviews.map(function (pr) {
            return '<div class="peer-item">'
                + '<div class="peer-header"><strong>' + escapeHtml(pr.reviewerName || 'Anonymous') + '</strong>'
                + (pr.score != null ? '<span class="peer-score">' + pr.score + '/5</span>' : '')
                + '</div>'
                + (pr.comment ? '<p class="peer-comment">' + escapeHtml(pr.comment) + '</p>' : '')
                + '</div>';
        }).join('');
    }
    ...
    function assignPeerReview() {
        ...
        var reviewers = ['Ahmed Hassan', 'Mariam Khalid', 'Youssef Ibrahim', 'Laila Mostafa'];
        var shuffled = reviewers.sort(function () { return 0.5 - Math.random(); });
        var selected = shuffled.slice(0, 2);

        currentSubmission.peerReviews = (currentSubmission.peerReviews || []).concat(selected.map(function (name) {
            return { reviewerName: name, score: null, comment: null, status: 'pending' };
        }));

window.NibrasReact.run(function() {
    var isEditMode = false, roleId = null;

    var PERMISSION_GROUPS = [
        { name: 'Course Management', perms: ['course:create', 'course:read', 'course:update', 'course:delete', 'section:create', 'section:enroll', 'section:remove-student'] },
        { name: 'User Management', perms: ['user:read', 'user:update', 'user:ban', 'user:adjust-points', 'user:bulk-create'] },
        { name: 'Role Management', perms: ['role:create', 'role:read', 'role:update', 'role:assign'] },
        { name: 'System', perms: ['backup:trigger', 'backup:restore', 'backup:list', 'audit-log:read', 'config:read', 'config:update'] },
        { name: 'Content', perms: ['content:moderate', 'badge:manage', 'flag:resolve'] },
    ];

    function gi(n){if(!n)return 'A';return n.split(' ').map(function(s){return s.charAt(0)}).join('').toUpperCase().slice(0,2)||'A';}
    ...
    function redirectWithToast(url, msg, type) {
        try { localStorage.setItem('adminToast', JSON.stringify({message:msg, type:type||'success'})); } catch(_){}
        ...
        PERMISSION_GROUPS.forEach(function(group){
            ...
            div.innerHTML =
                '<div class="perm-group-header">' +
                    ...
                    '<button type="button" class="group-toggle select-all" data-group="'+esc(group.name)+'">Select All</button>' +
                    '<button type="button" class="group-toggle deselect-all" data-group="'+esc(group.name)+'">Deselect All</button>' +
                '</div>' +
                '<div class="perm-grid" id="perm-grid-'+esc(group.name.replace(/\s+/g,'-'))+'"></div>';
            ...
            group.perms.forEach(function(perm){
                ...
                label.innerHTML =
                    '<input type="checkbox" name="permissions" value="'+esc(perm)+'">' +
                    '<span>'+esc(perm.replace(':', ': '))+'</span>' +
                    '<span class="perm-key">'+esc(perm)+'</span>';
    ...
    function initForm() {
        ...
        if (roleId) {
            ...
            document.getElementById('form-subtitle').textContent = 'Modify role details and permissions';
    ...
    async function loadRoleData(id) {
        ...
        try {
            ...
            document.getElementById('input-description').value = role.description || '';
    ...
    function initSubmit() {
        document.getElementById('role-form').addEventListener('submit', async function(e){
            ...
            try {
                if (isEditMode) {
                    await api('/admin/roles/'+encodeURIComponent(roleId), {method:'PATCH', body:body});
                    redirectWithToast('./roles.html', 'Role updated successfully', 'success');
                } else {
                    await api('/admin/roles', {method:'POST', body:body});
                    redirectWithToast('./roles.html', 'Role created successfully', 'success');
                }
            } catch(err) {
                hideLoading();

window.NibrasReact.run(function () {
    ...
    var ICONS = [
        'fa-star', 'fa-trophy', 'fa-medal', 'fa-crown', 'fa-gem',
        'fa-bolt', 'fa-fire', 'fa-rocket', 'fa-brain', 'fa-code',
        'fa-shield-halved', 'fa-wand-magic-sparkles', 'fa-book', 'fa-pen-fancy',
        'fa-graduation-cap', 'fa-flask', 'fa-microchip', 'fa-palette',
        'fa-music', 'fa-leaf', 'fa-heart', 'fa-handshake', 'fa-users',
        'fa-globe', 'fa-infinity'
    ];
    ...
    function renderIconPicker() {
        ...
        ICONS.forEach(function (icon) {
            ...
            div.addEventListener('click', function () {
                picker.querySelectorAll('.icon-option').forEach(function (el) { el.classList.remove('selected'); });
    ...
    function loadBadges() {
        container.innerHTML = '<div class="loading-state"><i class="fa-solid fa-spinner fa-spin"></i><p>Loading badges...</p></div>';

        if (!services || !services.gamificationService) {
            container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Gamification service not available.</p></div>';
            return;
        }

        services.gamificationService.getAllBadges().then(function (res) {
            var data = res && (res.data || res);
            var badges = Array.isArray(data) ? data : (Array.isArray(data.badges) ? data.badges : []);
            if (badges.length === 0) {
                container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-medal"></i><p>No badges created yet.</p></div>';
                return;
            }
            renderBadges(badges);
            ...
            container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Failed to load badges: ' + esc(err.message || 'Unknown error') + '</p></div>';
        });
    }
    ...
    function renderBadges(badges) {
        ...
        badges.forEach(function (b) {
            ...
            html += '<span class="badge-icon" style="color:' + esc(color) + ';"><i class="fa-solid ' + esc(icon) + '"></i></span>';
            html += '<div class="badge-name">' + esc(name) + '</div>';
            if (description) html += '<div class="badge-description">' + esc(description) + '</div>';
            html += '<div class="badge-meta">';
            html += '<span><i class="fa-solid fa-tag"></i> ' + esc(category) + '</span>';
            if (criteria) html += '<span><i class="fa-solid fa-list-check"></i> ' + esc(criteria) + '</span>';
            html += '<span><i class="fa-solid fa-users"></i> ' + esc(awardCount) + ' awarded</span>';
            html += '</div>';
            html += '<div class="badge-actions">';
            html += '<button class="btn-edit" data-id="' + esc(id) + '"><i class="fa-regular fa-pen-to-square"></i> Edit</button>';
            html += '<button class="btn-delete" data-id="' + esc(id) + '"><i class="fa-regular fa-trash-can"></i> Delete</button>';
            html += '</div>';
            html += '</div>';
        });
        html += '</div>';
    ...
    container.addEventListener('click', function (e) {
        ...
        services.gamificationService.deleteBadge(badgeId).then(function () {
    ...
    function validateForm() {
        submitBtn.disabled = !badgeName.value.trim() || !badgeDescription.value.trim();
    }
    ...
    openBtn.addEventListener('click', function () {
        ...
        setTimeout(function () { badgeName.focus(); }, 100);
    });
    ...
    submitBtn.addEventListener('click', function () {
        ...
        services.gamificationService.createBadge(payload).then(function () {
    ...
    awardBtn.addEventListener('click', function () {
        ...
        if (!badgeId) { showToast('Please select a badge.', 'error'); return; }
        if (!student) { showToast('Please enter a student email or ID.', 'error'); return; }
        ...
        services.gamificationService.checkAwardBadges(payload.studentId || payload.email).then(function () {

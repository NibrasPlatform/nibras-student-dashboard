window.NibrasReact.run(function() {
    ...
    async function loadConfig() {
        ...
        cc.innerHTML = '<div class="loading-state"><i class="fa-solid fa-spinner fa-spin"></i><p>Loading configuration...</p></div>';
        try {
            ...
            cc.innerHTML = '<div class="error-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Failed to load configuration: '+esc(err.message||'Unknown error')+'</p><p style="font-size:0.82rem;color:var(--text-secondary);margin-top:0.5rem;">Config endpoints may not be implemented on the backend yet.</p><button class="btn-primary" onclick="location.reload()" style="margin-top:1rem;"><i class="fa-solid fa-rotate"></i> Retry</button></div>';
        }
    }
    ...
    function renderConfig(config) {
        ...
        if (groups.length === 0) {
            cc.innerHTML = '<div class="config-empty-state"><i class="fa-solid fa-sliders"></i><p>No configuration data available</p></div>';
            return;
        }
        var html = '';
    ...
    function findConfigGroups(config) {
        var knownGroups = ['featureFlags', 'flags', 'gamification', 'gamificationRules', 'thresholds', 'reputationThresholds', 'reputation'];
        var groups = [];
        var seen = {};
        ...
        Object.keys(config).forEach(function(key){
            if (knownGroups.indexOf(key) === -1 && config[key] && isObject(config[key])) {
    ...
    function renderGroupCard(groupName, data) {
        ...
        keys.forEach(function(key){
            var val = data[key];
            if (isObject(val)) {
                rowsHtml += '<div class="config-row"><span class="config-label">'+esc(formatLabel(key))+'</span><span class="config-value" style="font-size:0.78rem;">object</span></div>';
                return;
            }
            var valDisplay, valClass = 'config-value';
            if (isBoolean(val)) {
                valDisplay = val ? '<span class="toggle-switch"><input type="checkbox" checked disabled><span class="toggle-slider"></span></span>' : '<span class="toggle-switch"><input type="checkbox" disabled><span class="toggle-slider"></span></span>';
                valClass += ' boolean';
            ...
            rowsHtml += '<div class="config-row" data-key="'+esc(key)+'">' +
                '<div><div class="config-label">'+esc(formatLabel(key))+'</div></div>' +
                '<div class="'+valClass+' config-view-value">'+valDisplay+'</div>' +
                '</div>';
        });
        return '<div class="config-card" data-group="'+esc(groupName)+'">' +
            '<div class="config-card-header">' +
            '<h3 class="config-card-title"><i class="'+icon+'"></i> '+esc(displayName)+'</h3>' +
            '<div class="config-card-actions">' +
            '<button class="btn-sm btn-sm-edit edit-btn" data-group="'+esc(groupName)+'"><i class="fa-solid fa-pen"></i> Edit</button>' +
            '</div>' +
            '</div>' +
            '<div class="config-card-body">'+rowsHtml+'</div>' +
            '</div>';
    }

    function enterEditMode(groupName) {
        var card = document.querySelector('.config-card[data-group="'+esc(groupName)+'"]');
        if (!card) return;
        ...
        card.querySelectorAll('.config-row').forEach(function(row){
            ...
            if (isBoolean(val)) {
                ...
                inputHtml = '<label class="toggle-switch"><input type="checkbox" class="config-edit-toggle" '+checked+'><span class="toggle-slider"></span></label>';
            } else if (isNumber(val)) {
                inputHtml = '<input type="number" class="config-edit-input number" value="'+esc(String(val))+'">';
            } else {
                inputHtml = '<input type="text" class="config-edit-input string" value="'+esc(String(val || ''))+'">';
            }
        ...
        if (actions) {
            actions.innerHTML =
                '<button class="btn-sm btn-sm-save save-btn" data-group="'+esc(groupName)+'"><i class="fa-solid fa-floppy-disk"></i> Save</button>' +
                '<button class="btn-sm btn-sm-cancel cancel-btn" data-group="'+esc(groupName)+'"><i class="fa-solid fa-xmark"></i> Cancel</button>';
            actions.querySelector('.save-btn').addEventListener('click', function(){ saveGroup(groupName); });
            actions.querySelector('.cancel-btn').addEventListener('click', function(){ cancelEdit(groupName); });
        }
    }
    ...
    function exitEditMode(groupName, newData) {
        ...
        var actions = card.querySelector('.config-card-actions');
        if (actions) {
            actions.innerHTML = '<button class="btn-sm btn-sm-edit edit-btn" data-group="'+esc(groupName)+'"><i class="fa-solid fa-pen"></i> Edit</button>';
            actions.querySelector('.edit-btn').addEventListener('click', function(){ enterEditMode(groupName); });
        }
    }

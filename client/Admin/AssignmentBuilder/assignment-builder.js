(function () {
    ...
    function init() {
        ...
        if (navLinkGrades && courseId) {
            navLinkGrades.href = '../InstructorGrades/instructor-grades.html?courseId=' + encodeURIComponent(courseId);
        }
    }

    function setupUI() {
        document.getElementById('btn-create-assignment').addEventListener('click', function () { openCreateModal(); });
        document.getElementById('modal-close').addEventListener('click', closeModal);
        document.getElementById('btn-cancel').addEventListener('click', closeModal);
        document.getElementById('btn-save').addEventListener('click', saveAssignment);
        document.getElementById('btn-add-test-case').addEventListener('click', function () { addTestCaseRow(); });
        document.getElementById('btn-add-rubric').addEventListener('click', function () { addRubricRow(); });
        document.getElementById('btn-batch-import').addEventListener('click', function () {
            document.getElementById('batch-file-input').click();
        });
        document.getElementById('batch-file-input').addEventListener('change', handleBatchImport);
    ...
    function loadAssignments() {
        ...
        if (!S || !S.assignmentAdminService) {
            container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-circle-exclamation"></i><p>Assignment service unavailable.</p></div>';
            return;
        }
    ...
    function renderAssignments() {
        ...
        if (!assignments.length) {
            container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-file-circle-plus"></i><p>No assignments yet. Click "Create Assignment" to get started.</p></div>';
            return;
        }

        container.innerHTML = assignments.map(function (a) {
            var due = a.dueDate ? new Date(a.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'No due date';
            var typeLabel = a.type || 'code';
            var typeIcon = typeLabel === 'code' ? 'fa-solid fa-code' : typeLabel === 'quiz' ? 'fa-solid fa-question' : 'fa-solid fa-folder-open';
            return '<div class="assignment-card" data-id="' + (a._id || a.id) + '">'
                + '<div class="card-top">'
                + '<div class="card-icon"><i class="' + typeIcon + '"></i></div>'
                + '<div class="card-info">'
                + '<h3>' + escapeHtml(a.title || 'Untitled') + '</h3>'
                + '<p>' + escapeHtml(a.description || '') + '</p>'
                + '</div>'
                + '<div class="card-points">' + (a.points || a.maxScore || '—') + ' pts</div>'
                + '</div>'
                + '<div class="card-meta">'
                + '<span><i class="fa-regular fa-calendar"></i> ' + due + '</span>'
                + '<span><i class="fa-solid fa-vial"></i> ' + (a.testCaseCount || 0) + ' tests</span>'
                + '</div>'
                + '<div class="card-actions">'
                + '<button class="action-btn edit-btn" data-id="' + (a._id || a.id) + '"><i class="fa-regular fa-pen-to-square"></i> Edit</button>'
                + '<button class="action-btn delete-btn" data-id="' + (a._id || a.id) + '"><i class="fa-regular fa-trash-can"></i> Delete</button>'
                + '</div>'
                + '</div>';
        }).join('');
        ...
        container.querySelectorAll('.delete-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                var id = this.getAttribute('data-id');
                if (confirm('Delete this assignment? This cannot be undone.')) deleteAssignment(id);
            });
        });
    }
    ...
    function openCreateModal() {
        ...
        document.getElementById('btn-save').innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Assignment';
        resetForm();
        document.getElementById('assignment-modal').style.display = 'flex';
    }

    function openEditModal(id) {
        var a = assignments.find(function (x) { return (x._id || x.id) === id; });
        if (!a) return;
        editingId = id;
        document.getElementById('modal-title').textContent = 'Edit Assignment';
        document.getElementById('btn-save').innerHTML = '<i class="fa-solid fa-pen-to-square"></i> Update Assignment';
        ...
        document.getElementById('field-points').value = a.points || a.maxScore || 100;
        ...
        if (a.resourceLimits) {
            ...
            document.getElementById('field-memory').value = a.resourceLimits.memory || 256;
            document.getElementById('field-time-limit').value = a.resourceLimits.time || 5;
    ...
    function collectTestCases() {
        ...
        rows.forEach(function (row) {
            ...
            var memoryLimit = parseInt(row.querySelector('.tc-memory-limit')?.value) || 256;
            cases.push({ input: input, expectedOutput: expected, isHidden: isHidden, weight: weight, timeLimit: timeLimit, memoryLimit: memoryLimit });
        });
        return cases;
    }
    ...
    function collectRubric() {
        ...
        rows.forEach(function (row) {
            ...
            if (name) criteria.push({ name: name, description: desc, maxPoints: maxPts });
        });
        return criteria;
    }
    ...
    function addTestCaseRow(tc) {
        ...
        div.id = id;
        div.innerHTML = '<div class="tc-fields">'
            + '<div class="tc-field"><label class="label-sm">Input</label><textarea class="form-input-sm tc-input" rows="2">' + escapeHtml(tc?.input || '') + '</textarea></div>'
            + '<div class="tc-field"><label class="label-sm">Expected Output</label><textarea class="form-input-sm tc-expected" rows="2">' + escapeHtml(tc?.expectedOutput || '') + '</textarea></div>'
            + '</div>'
            + '<div class="tc-options">'
            + '<label class="tc-check-label"><input type="checkbox" class="tc-hidden" ' + (tc?.isHidden ? 'checked' : '') + '> Hidden</label>'
            + '<div><label class="label-sm">Weight</label><input type="number" class="form-input-xs tc-weight" value="' + (tc?.weight || 1) + '" min="0.1" step="0.1"></div>'
            + '<div><label class="label-sm">Time (s)</label><input type="number" class="form-input-xs tc-time-limit" value="' + (tc?.timeLimit || 5) + '" min="1"></div>'
            + '<div><label class="label-sm">Memory (MB)</label><input type="number" class="form-input-xs tc-memory-limit" value="' + (tc?.memoryLimit || 256) + '" min="16"></div>'
            + '<button class="tc-remove" title="Remove test case"><i class="fa-solid fa-xmark"></i></button>'
            + '</div>';
        container.appendChild(div);
        div.querySelector('.tc-remove').addEventListener('click', function () { div.remove(); });
    }
    ...
    function renderTestCases(testCases) {
        ...
        if (!testCases || !testCases.length) {
            container.innerHTML = '<p class="empty-hint">No test cases yet. Add one above.</p>';
            return;
        }
        testCases.forEach(function (tc) { addTestCaseRow(tc); });
    }
    ...
    function handleBatchImport(e) {
        ...
        reader.onload = function (ev) {
            try {
                ...
                cases.forEach(function (tc) { addTestCaseRow(tc); });
            } catch (_) {
                // Try CSV
                try {
                    ...
                    for (var i = 1; i < lines.length; i++) {
                        ...
                        if (parts.length >= 2) {
                            addTestCaseRow({ input: parts[0].trim(), expectedOutput: parts[1].trim(), weight: parseFloat(parts[2]) || 1 });
                        }
                    }
                } catch (_2) {
    ...
    function addRubricRow(criterion) {
        ...
        div.id = id;
        div.innerHTML = '<div class="rb-fields">'
            + '<div class="rb-field rb-field-wide"><label class="label-sm">Name</label><input type="text" class="form-input-sm rb-name" placeholder="e.g. Correctness" value="' + escapeHtml(criterion?.name || '') + '"></div>'
            + '<div class="rb-field"><label class="label-sm">Max Points</label><input type="number" class="form-input-sm rb-max" value="' + (criterion?.maxPoints || 10) + '" min="0"></div>'
            + '<button class="rb-remove" title="Remove criterion"><i class="fa-solid fa-xmark"></i></button>'
            + '</div>'
            + '<div class="rb-field"><label class="label-sm">Description</label><input type="text" class="form-input-sm rb-desc" placeholder="What this criterion measures" value="' + escapeHtml(criterion?.description || '') + '"></div>';
        container.appendChild(div);
        div.querySelector('.rb-remove').addEventListener('click', function () { div.remove(); });
    }
    ...
    function renderRubric(criteria) {
        ...
        if (!criteria || !criteria.length) {
            container.innerHTML = '<p class="empty-hint">No rubric criteria yet. Add one above.</p>';
            return;
        }
        criteria.forEach(function (c) { addRubricRow(c); });
    }

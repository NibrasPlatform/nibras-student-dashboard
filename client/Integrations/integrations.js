window.NibrasReact.run(() => {
    ...
    async function apiRequestWithFallback(path, options = {}, fallbackStatuses = [404, 405, 501]) {
        ...
        for (let i = 0; i < candidates.length; i += 1) {
            ...
            try {
                return await apiFetch(path, Object.assign({}, options, { service: candidate.service }));
            } catch (error) {
    ...
    function setStatus(id, message, tone) {
        ...
        el.style.color = tone === 'error' ? 'var(--tag-red-text, #dc2626)' : tone === 'success' ? 'var(--accent-blue, #2563eb)' : '';
    }
    ...
    function loadUserProfile() {
        ...
        try {
            ...
            apiFetch('/auth/me', { service: 'admin', method: 'GET', auth: true }).then(function (data) {
    ...
    function applyUserDataToUI() {
        ...
        if (displayName) {
            ...
            if (avatarCircle && avatarCircle.textContent.trim() === 'ZA') avatarCircle.textContent = initials;
            var sidebarNames = document.querySelectorAll('.user-profile h4');
            sidebarNames.forEach(function (el) {
                if (el.textContent.trim() === 'Ziad Alaa' || el.textContent.trim() === '') el.textContent = displayName;
            });
            ...
            sidebarRoles.forEach(function (el) {
                if (el.textContent.trim() === 'student' || el.textContent.trim() === '') el.textContent = displayRole;
            });
        }
    ...
    document.getElementById('btn-canvas-connect')?.addEventListener('click', async function () {
        var btn = this;
        var orig = btn.innerHTML;
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Connecting...';
        try {
            var url = adminApiBase.replace(/\/api\/?$/, '') + '/api/integrations/canvas/connect';
            var returnTo = window.location.href;
            window.location.href = url + '?return_to=' + encodeURIComponent(returnTo);
        } catch (err) {
            setStatus('canvas-status-text', 'Connection failed: ' + (err.message || 'Unknown'), 'error');
            btn.disabled = false;
            btn.innerHTML = orig;
        }
    });

    document.getElementById('btn-canvas-sync')?.addEventListener('click', async function () {
        var btn = this;
        var orig = btn.innerHTML;
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Syncing...';
        try {
            await apiRequestWithFallback('/api/integrations/canvas/sync-roster', { method: 'POST', auth: true });
            setStatus('canvas-status-text', 'Roster synced successfully', 'success');
        } catch (err) {
            setStatus('canvas-status-text', 'Sync failed: ' + (err.message || 'Error'), 'error');
        } finally {
            btn.disabled = false;
            btn.innerHTML = orig;
        }
    ...
    document.getElementById('btn-canvas-disconnect')?.addEventListener('click', async function () {
        if (!confirm('Disconnect Canvas LMS integration?')) return;
        try {
            await apiRequestWithFallback('/api/integrations/canvas/disconnect', { method: 'POST', auth: true });
            location.reload();
        } catch (err) {
            setStatus('canvas-status-text', 'Disconnect failed: ' + (err.message || 'Error'), 'error');
        }
    });
    ...
    document.getElementById('btn-moodle-toggle')?.addEventListener('click', function () {
        var detail = document.getElementById('moodle-detail');
        detail.classList.toggle('visible');
    });

    document.getElementById('btn-moodle-test')?.addEventListener('click', async function () {
        var btn = this;
        ...
        try {
            await apiRequestWithFallback('/api/integrations/moodle/test', { method: 'POST', auth: true, body: { url: url, token: token } });
    ...
    document.getElementById('btn-moodle-connect')?.addEventListener('click', async function () {
        var btn = this;
        var url = document.getElementById('moodle-url')?.value?.trim();
        var token = document.getElementById('moodle-token')?.value?.trim();
        var resultEl = document.getElementById('moodle-test-result');
        ...
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Connecting...';
        try {
            await apiRequestWithFallback('/api/integrations/moodle/connect', { method: 'POST', auth: true, body: { url: url, token: token } });
    ...
    document.getElementById('btn-moodle-disconnect')?.addEventListener('click', async function () {
        if (!confirm('Disconnect Moodle integration?')) return;
        try {
            await apiRequestWithFallback('/api/integrations/moodle/disconnect', { method: 'POST', auth: true });
            location.reload();
        } catch (err) {
            setStatus('moodle-status-text', 'Disconnect failed: ' + (err.message || 'Error'), 'error');
        }
    });
    ...
    function loadSlackStatus(u) {
        ...
        if (hasSlack) {
            ...
            if (connectBtn) connectBtn.style.display = 'none';
            if (detailArea) detailArea.classList.add('visible');
            if (workspaceName) workspaceName.textContent = u.slackWorkspace || u.slackTeamName || 'Connected';
        } else {
    ...
    document.getElementById('btn-slack-connect')?.addEventListener('click', async function () {
        var btn = this;
        var orig = btn.innerHTML;
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Connecting...';
        try {
            var returnTo = window.location.href;
            var url = adminApiBase.replace(/\/api\/?$/, '') + '/api/integrations/slack/connect';
            window.location.href = url + '?return_to=' + encodeURIComponent(returnTo);
        } catch (err) {
            setStatus('slack-status-text', 'Connection failed: ' + (err.message || 'Unknown'), 'error');
            btn.disabled = false;
            btn.innerHTML = orig;
        }
    });

    document.getElementById('btn-slack-disconnect')?.addEventListener('click', async function () {
        if (!confirm('Disconnect Slack integration?')) return;
        try {
            await apiRequestWithFallback('/api/integrations/slack/disconnect', { method: 'POST', auth: true });
            location.reload();
        } catch (err) {
            setStatus('slack-status-text', 'Disconnect failed: ' + (err.message || 'Error'), 'error');
        }
    });
    ...
    function loadDiscordStatus(u) {
        var hasDiscord = Boolean(u.discordLinked || u.discordWebhookUrl || u.discordConfigured);
    ...
    document.getElementById('btn-discord-toggle')?.addEventListener('click', function () {
        var detail = document.getElementById('discord-detail');
        detail.classList.toggle('visible');
    });

    document.getElementById('btn-discord-test')?.addEventListener('click', async function () {
        var btn = this;
        ...
        resultEl.textContent = '';
        try {
            await apiRequestWithFallback('/api/integrations/discord/test', { method: 'POST', auth: true, body: { webhookUrl: url } });
    ...
    document.getElementById('btn-discord-save')?.addEventListener('click', async function () {
        var btn = this;
        var url = document.getElementById('discord-webhook-url')?.value?.trim();
        var resultEl = document.getElementById('discord-test-result');
        if (!url) {
        ...
        var events = [];
        document.querySelectorAll('#discord-events .channel-event-tag.active').forEach(function (tag) {
        ...
        try {
            await apiRequestWithFallback('/api/integrations/discord/connect', { method: 'POST', auth: true, body: { webhookUrl: url, events: events } });
    ...
    document.getElementById('btn-discord-disconnect')?.addEventListener('click', async function () {
        if (!confirm('Disconnect Discord integration?')) return;
        try {
            await apiRequestWithFallback('/api/integrations/discord/disconnect', { method: 'POST', auth: true });
            location.reload();
        } catch (err) {
            setStatus('discord-status-text', 'Disconnect failed: ' + (err.message || 'Error'), 'error');
        }
    });
    ...
    function loadGitHubStatus(u) {
        var hasGitHub = Boolean(u.githubId || u.githubUsername || u.githubLinked || u.githubAppInstalled);
        ...
        if (hasGitHub) {
            ...
            if (badge) { badge.className = 'integration-status connected'; badge.innerHTML = '<i class="fa-solid fa-circle"></i> Webhook Active'; }
            if (detailArea) detailArea.classList.add('visible');
            if (avatar && u.githubAvatarUrl) { avatar.src = u.githubAvatarUrl; avatar.style.display = ''; }
            if (connectedAs) connectedAs.textContent = 'Connected as ' + (u.githubUsername || 'GitHub user');
        } else {
            setStatus('github-status-text', 'Not connected');
            if (badge) { badge.className = 'integration-status disconnected'; badge.innerHTML = '<i class="fa-solid fa-circle"></i> Webhook Inactive'; }
            if (detailArea) detailArea.classList.remove('visible');
        }
    }
    ...
    function loadGitLabStatus(u) {
        var hasGitLab = Boolean(u.gitlabLinked || u.gitlabId || u.gitlabWebhookConfigured);
        ...
        if (hasGitLab) {
            ...
            if (badge) { badge.className = 'integration-status connected'; badge.innerHTML = '<i class="fa-solid fa-circle"></i> Configured'; }
            if (detailArea) detailArea.classList.add('visible');
            if (configureBtn) configureBtn.textContent = 'Settings';
        } else {
            setStatus('gitlab-status-text', 'Not configured');
            if (badge) { badge.className = 'integration-status disconnected'; badge.innerHTML = '<i class="fa-solid fa-circle"></i> Not Configured'; }
            if (detailArea) detailArea.classList.remove('visible');
        }
    }

    document.getElementById('btn-gitlab-configure')?.addEventListener('click', function () {
        var detail = document.getElementById('gitlab-detail');
        detail.classList.toggle('visible');
    });

    document.getElementById('btn-gitlab-copy-url')?.addEventListener('click', function () {
    ...
    (function handleOAuthCallback() {
        ...
        if (integration && status) {
            if (status === 'success') {
                setStatus(integration + '-status-text', message || 'Connected successfully', 'success');
            } else if (status === 'error') {
                setStatus(integration + '-status-text', message || 'Connection failed', 'error');
            }

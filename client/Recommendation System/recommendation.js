(function runRecommendationPage(initializer) {
    ...
    const TRACK_DESCRIPTIONS = Object.freeze({
        'Artificial Intelligence': 'Build intelligent systems that learn from data and make predictions.',
        'Information Track': 'Focus on data-centric systems, analytics, and information management.',
        'Visual Computing': 'Study computer graphics, vision, and image understanding techniques.',
        'Systems': 'Design reliable software systems, infrastructure, and performance-focused solutions.',
        'Theory': 'Dive into mathematical foundations and algorithmic reasoning in computer science.',
        'Computer Engineering': 'Combine hardware and software to build integrated computing platforms.',
        'Human-Computer Interaction': 'Design user-centered systems with strong usability and UX outcomes.',
        'Computational Biology': 'Apply computation and data science to biological and health problems.',
    });
    ...
    const getRankMeta = (rank) => {
        ...
        if (rank === 2) return { rankClass: 'rank-silver', matchClass: 'match-medium' };
    ...
    const toCourseCanonicalCode = (value) => {
        ...
        if (COURSE_ALIAS_TO_CODE[rawNoSymbols]) return COURSE_ALIAS_TO_CODE[rawNoSymbols];
        ...
        for (let i = 0; i < patterns.length; i += 1) {
            ...
            if (!match) continue;
            const candidate = String(match[1] || match[0]).toUpperCase().replace(/[^A-Z0-9]/g, '');
            if (COURSE_ALIAS_TO_CODE[candidate]) return COURSE_ALIAS_TO_CODE[candidate];
        }
    ...
    const extractGradeEntries = (payload) => {
        ...
        walkObject(payload, (node) => {
            const codeValue = pickValue(node, ['courseCode', 'course_code', 'code', 'courseId', 'courseID', 'courseName', 'course', 'name', 'title']);
            const directCode = typeof codeValue === 'object'
                ? pickValue(codeValue, ['code', 'courseCode', 'course_code', 'name', 'title'])
                : codeValue;
            const courseCode = toCourseCanonicalCode(directCode);
            if (!courseCode) return;

            let gradeValue = pickValue(node, ['grade', 'finalGrade', 'score', 'percentage', 'percent', 'mark', 'value']);
            if (gradeValue && typeof gradeValue === 'object') {
                gradeValue = pickValue(gradeValue, ['value', 'grade', 'score', 'percentage', 'percent']);
            }
    ...
    const extractTrackReason = (trackName, explanationText) => {
        const text = String(explanationText || '').replace(/\n+/g, ' ').trim();
        if (!text) return '';
        const target = String(trackName || '').toLowerCase();
        const sentences = text.split(/(?<=[.!?])\s+/).map((s) => s.trim()).filter(Boolean);
        const matched = sentences.filter((sentence) => sentence.toLowerCase().includes(target));
    ...
    const formatExplanationToHtml = (rawText) => {
        ...
        const flushList = () => {
            ...
            htmlParts.push(`<ul class="explanation-list">${currentList.join('')}</ul>`);
            currentList = [];
        };
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            if (line.startsWith('## ')) {
                flushList();
                htmlParts.push(`<h5 class="explanation-heading">${escapeHtml(line.slice(3))}</h5>`);
                continue;
            }
            ...
            flushList();
            htmlParts.push(`<p class="explanation-paragraph">${escapeHtml(line)}</p>`);
        }
    ...
    const explanationBody = document.getElementById('recommendation-explanation-content');
    ...
    const buildTrackCards = (result, gradesCount) => {
        if (!trackContainer) return;
        const tracks = Array.isArray(result?.recommendations) ? result.recommendations : [];
        if (tracks.length === 0) {
        ...
        trackContainer.innerHTML = tracks.map((trackName, index) => {
            const rank = index + 1;
            const rankMeta = getRankMeta(rank);
            const safeTrackName = String(trackName || '').trim() || `Track ${rank}`;
            const trackDescription = TRACK_DESCRIPTIONS[safeTrackName] || 'Recommended based on your current academic capability profile.';
            const matchPercent = Number.isFinite(matchMap[safeTrackName.toLowerCase()])
                ? Number(matchMap[safeTrackName.toLowerCase()].toFixed(2))
                : fallbackMatch[index] || 60;
            const reasonText = extractTrackReason(safeTrackName, explanation) || `This track aligns with your strongest capabilities: ${strengths.join(', ') || 'available skills'}.`;
            const strengthsHtml = strengths.length
                ? strengths.map((item) => `<span class="strength-chip">${escapeHtml(item)}</span>`).join('')
    ...
    const loadBackendGrades = async () => {
        ...
        if (recommendationService && typeof recommendationService.getGradesPayload === 'function') {
            ...
            let grades = extractGradeEntries(firstPayloadResult?.payload || firstPayloadResult);
            if (Object.keys(grades).length > 0) {
                return { grades, source: firstPayloadResult?.source || 'backend API' };
            }

            const refreshedPayloadResult = await recommendationService.getGradesPayload({ refreshSheet: true });
            grades = extractGradeEntries(refreshedPayloadResult?.payload || refreshedPayloadResult);
            if (Object.keys(grades).length > 0) {
                return { grades, source: refreshedPayloadResult?.source || 'backend API (refreshed)' };
            }
        }
        ...
        if (programService && typeof programService.getSheet === 'function') {
            ...
            if (Object.keys(grades).length > 0) return { grades, source: 'tracking:/v1/programs/student/me/sheet' };
            ...
            if (typeof programService.generateSheet === 'function') {
                ...
                if (Object.keys(grades).length > 0) return { grades, source: 'tracking:/v1/programs/student/me/sheet (generated)' };
            }
        }

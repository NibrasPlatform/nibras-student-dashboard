(function () {
    ...
    function showNotAuthorized() {
        ...
        overlay.innerHTML = '<div style="background:#1e1e2e;border-radius:12px;padding:2rem 3rem;text-align:center;max-width:420px;box-shadow:0 8px 32px rgba(0,0,0,0.4);">' +
            ...
            '<button onclick="window.location.replace(\'' + loginPage + '\')" style="background:#6c5ce7;color:#fff;border:none;border-radius:8px;padding:0.75rem 2rem;cursor:pointer;font-size:1rem;">Go to Login</button>' +
            '</div>';
    ...
    var isAdminPage = ADMIN_PATHS.some(function (p) { return currentPath.indexOf(p) !== -1; });
    ...
    if (isAdminPage) {
        ...
        if (storedUser) {
            var role = String(storedUser.role?.name || storedUser.role || '').toLowerCase();
